...
#define XAUDIO2_INVALID_OPSET (uint32_t)(-1) // Not allowed for OperationSet arguments
...
{
	uint32_t CreationFlags; // flags the voice was created with.
	uint32_t InputChannels; // Channels in the voice's input audio.
	uint32_t InputSampleRate; // Sample rate of the voice's input audio.
} XAUDIO2_VOICE_DETAILS;

// Used in XAUDIO2_VOICE_SENDS below
typedef struct XAUDIO2_SEND_DESCRIPTOR
{
	uint32_t flags; // Either 0 or XAUDIO2_SEND_USEFILTER.
...
{
	uint32_t SendCount; // Number of sends from this voice.
...
typedef struct XAUDIO2_EFFECT_DESCRIPTOR
{
	...
	uint32_t OutputChannels; // How many output channels the effect should produce.
} XAUDIO2_EFFECT_DESCRIPTOR;

// Used in the voice creation functions and in IXAudio2Voice::SetEffectChain
typedef struct XAUDIO2_EFFECT_CHAIN
{
	uint32_t EffectCount; // Number of effects in this voice's effect chain.
...
{
	uint32_t flags; // Either 0 or XAUDIO2_END_OF_STREAM.
	uint32_t AudioBytes; // Size of the audio data buffer in bytes.
	const uint8_t* pAudioData; // Pointer to the audio data buffer.
	uint32_t PlayBegin; // First sample in this buffer to be played.
	uint32_t PlayLength; // Length of the region to be played in samples,
		//  or 0 to play the whole buffer.
	uint32_t LoopBegin; // First sample of the region to be looped.
	uint32_t LoopLength; // Length of the desired loop region in samples,
		//  or 0 to loop the entire buffer.
	uint32_t LoopCount; // Number of times to repeat the loop region,
...
{
	const uint32_t* pDecodedPacketCumulativeBytes; // Decoded packet's cumulative size array.
	...
	uint32_t PacketCount; // Number of XWMA packets submitted. Must be >= 1 and
...
typedef struct XAUDIO2_VOICE_STATE
{
	...
	uint32_t BuffersQueued; // Number of buffers currently queued on the voice
		//  (including the one that is being processed).
	uint64_t SamplesPlayed; // Total number of samples produced by the voice since
...
typedef struct XAUDIO2_PERFORMANCE_DATA
{
	...
	uint64_t AudioCyclesSinceLastQuery; // CPU cycles spent on audio processing since the
		//  last call to StartEngine or GetPerformanceData.
	uint64_t TotalCyclesSinceLastQuery; // Total CPU cycles elapsed since the last call
		//  (only counts the CPU XAudio2 is running on).
	uint32_t MinimumCyclesPerQuantum; // Fewest CPU cycles spent processing any one
		//  audio quantum since the last call.
	uint32_t MaximumCyclesPerQuantum; // Most CPU cycles spent processing any one
		//  audio quantum since the last call.

	// Memory usage information
	uint32_t MemoryUsageInBytes; // Total heap space currently in use.

	// Audio latency and glitching information
	uint32_t CurrentLatencyInSamples; // Minimum delay from when a sample is read from a
		//  source buffer to when it reaches the speakers.
	uint32_t GlitchesSinceEngineStarted; // Audio dropouts since the engine was started.

	// Data about XAudio2's current workload
	uint32_t ActiveSourceVoiceCount; // Source voices currently playing.
	uint32_t TotalSourceVoiceCount; // Source voices currently existing.
	uint32_t ActiveSubmixVoiceCount; // Submix voices currently playing/existing.

	uint32_t ActiveResamplerCount; // Resample xAPOs currently active.
	uint32_t ActiveMatrixMixCount; // MatrixMix xAPOs currently active.

	// Usage of the hardware XMA decoder (Xbox 360 only)
	uint32_t ActiveXmaSourceVoices; // Number of source voices decoding XMA data.
	uint32_t ActiveXmaStreams; // A voice can use more than one XMA stream.
} XAUDIO2_PERFORMANCE_DATA;

// Used in IXAudio2::SetDebugConfiguration
typedef struct XAUDIO2_DEBUG_CONFIGURATION
{
	uint32_t TraceMask; // Bitmap of enabled debug message types.
	uint32_t BreakMask; // message types that will break into the debugger.
...
DECLARE_INTERFACE_(IXAudio2, IUnknown)
{
	...
	//
	STDMETHOD(GetDeviceCount)
	(THIS_ __out uint32_t * pCount) PURE;
	...
	//
	STDMETHOD(GetDeviceDetails)
	(THIS_ uint32_t Index, __out XAUDIO2_DEVICE_DETAILS * pDeviceDetails) PURE;
	...
	(THIS_ uint32_t flags X2DEFAULT(0),
	...
	//
	STDMETHOD(RegisterForCallbacks)
	(__in IXAudio2EngineCallback * pCallback) PURE;
	...
	(THIS_ __deref_out IXAudio2SourceVoice * *ppSourceVoice,
		__in const WAVEFORMATEX* pSourceFormat,
		uint32_t flags X2DEFAULT(0),
	...
	(THIS_ __deref_out IXAudio2SubmixVoice * *ppSubmixVoice,
		uint32_t InputChannels, uint32_t InputSampleRate,
		uint32_t flags X2DEFAULT(0), uint32_t ProcessingStage X2DEFAULT(0),
		__in_opt const XAUDIO2_VOICE_SENDS* pSendList X2DEFAULT(NULL),
		__in_opt const XAUDIO2_EFFECT_CHAIN* pEffectChain X2DEFAULT(NULL)) PURE;
	...
	(THIS_ __deref_out IXAudio2MasteringVoice * *ppMasteringVoice,
		uint32_t InputChannels X2DEFAULT(XAUDIO2_DEFAULT_CHANNELS),
		uint32_t InputSampleRate X2DEFAULT(XAUDIO2_DEFAULT_SAMPLERATE),
		uint32_t flags X2DEFAULT(0), uint32_t DeviceIndex X2DEFAULT(0),
		__in_opt const XAUDIO2_EFFECT_CHAIN* pEffectChain X2DEFAULT(NULL)) PURE;
	...
	(THIS_ uint32_t OperationSet) PURE;
	...
	(THIS_ __out XAUDIO2_PERFORMANCE_DATA * pPerfData) PURE;
	...
	(THIS_ __in_opt const XAUDIO2_DEBUG_CONFIGURATION* pDebugConfiguration,
...
#define Declare_IXAudio2Voice_Methods()                                               \
    ...
    */                                           \
	STDMETHOD_(void, GetVoiceDetails)                                                 \
	(THIS_ __out XAUDIO2_VOICE_DETAILS * pVoiceDetails) PURE;                         \
                                                                                      \
    ...
    //  pSendList - Optional list of voices this voice should send audio to.
    */                                           \
	STDMETHOD(SetOutputVoices)                                                        \
	(THIS_ __in_opt const XAUDIO2_VOICE_SENDS* pSendList) PURE;                       \
                                                                                      \
    ...
    */                                            \
	...
	(THIS_ __in_opt const XAUDIO2_EFFECT_CHAIN* pEffectChain) PURE;                   \
                                                                                      \
    ...
    */                                              \
	STDMETHOD(EnableEffect)                                                           \
	(THIS_ uint32_t EffectIndex,                                                      \
		uint32_t OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;                    \
                                                                                      \
    ...
    */                                            \
	STDMETHOD_(void, GetEffectState)                                                  \
	(THIS_ uint32_t EffectIndex, __out BOOL * pEnabled) PURE;                         \
                                                                                      \
    ...
    */                                       \
	...
	(THIS_ uint32_t EffectIndex,                                                      \
		...
		uint32_t ParametersByteSize) PURE;                                            \
                                                                                      \
    ...
    */                                       \
	...
	(THIS_ __in const XAUDIO2_FILTER_PARAMETERS* pParameters,                         \
		uint32_t OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;                    \
                                                                                      \
    ...
    */                                       \
	...
	(THIS_ __out XAUDIO2_FILTER_PARAMETERS * pParameters) PURE;                       \
                                                                                      \
    ...
    */                                 \
	...
	(THIS_ __in_opt IXAudio2Voice * pDestinationVoice,                                \
    ...
    */                                                 \
	STDMETHOD_(void, GetVolume)                                                       \
	(THIS_ __out float* pVolume) PURE;                                                \
                                                                                      \
    ...
    */                                         \
	...
	(THIS_ uint32_t Channels, __in_ecount(Channels) const float* pVolumes,            \
		uint32_t OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;                    \
                                                                                      \
    ...
    */                                         \
	STDMETHOD_(void, GetChannelVolumes)                                               \
	(THIS_ uint32_t Channels, __out_ecount(Channels) float* pVolumes) PURE;           \
                                                                                      \
    ...
    //  OperationSet - Used to identify this call as part of a deferred batch.
    */                                           \
	STDMETHOD(SetOutputMatrix)                                                        \
	(THIS_ __in_opt IXAudio2Voice * pDestinationVoice,                                \
		uint32_t SourceChannels, uint32_t DestinationChannels,                        \
    ...
    */                                           \
	STDMETHOD_(void, GetOutputMatrix)                                                 \
	(THIS_ __in_opt IXAudio2Voice * pDestinationVoice,                                \
		uint32_t SourceChannels, uint32_t DestinationChannels,                        \
...
DECLARE_INTERFACE_(IXAudio2SourceVoice, IXAudio2Voice)
{
	...
	//
	STDMETHOD(Start)
	(THIS_ uint32_t flags X2DEFAULT(0), uint32_t OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;
	...
	(THIS_ __in const XAUDIO2_BUFFER* pBuffer, __in_opt const XAUDIO2_BUFFER_WMA* pBufferWMA X2DEFAULT(NULL)) PURE;

	// NAME: IXAudio2SourceVoice::FlushSourceBuffers
	// DESCRIPTION: Removes all pending audio buffers from this voice's queue.
	//
	STDMETHOD(FlushSourceBuffers)
	(THIS) PURE;
	...
	//
	STDMETHOD(ExitLoop)
	(THIS_ uint32_t OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;
	...
	//
	STDMETHOD_(void, GetState)
	(THIS_ __out XAUDIO2_VOICE_STATE * pVoiceState) PURE;
	...
	//
	STDMETHOD_(void, GetFrequencyRatio)
	(THIS_ __out float* pRatio) PURE;
	...
	//
	// ARGUMENTS:
	//  uint32_t - The intended sample rate of further submitted source data.
	//
	STDMETHOD(SetSourceSampleRate)
	(THIS_ uint32_t NewSourceSampleRate) PURE;
};
...
DECLARE_INTERFACE(IXAudio2EngineCallback)
{
	...
	STDMETHOD_(void, OnCriticalError)
	(THIS_ HRESULT Error) PURE;
};
...
DECLARE_INTERFACE(IXAudio2VoiceCallback)
{
	...
	(THIS_ uint32_t BytesRequired) PURE;
	...
	(THIS_ void* pBufferContext) PURE;
...
__inline float XAudio2CutoffFrequencyToRadians(float CutoffFrequency, uint32_t SampleRate)
{
...
STDAPI XAudio2Create(__deref_out IXAudio2** ppXAudio2, uint32_t flags X2DEFAULT(0),
	XAUDIO2_PROCESSOR XAudio2Processor X2DEFAULT(XAUDIO2_DEFAULT_PROCESSOR));

#else // Windows

__inline HRESULT XAudio2Create(__deref_out IXAudio2** ppXAudio2, uint32_t flags X2DEFAULT(0),
	XAUDIO2_PROCESSOR XAudio2Processor X2DEFAULT(XAUDIO2_DEFAULT_PROCESSOR))
{
