...
//
MLR_I_PMesh*
MLR_I_PMesh::Make(std::iostream stream, uint32_t version)
{
	Check_Object(stream);
#ifdef _GAMEOS_HPP_
...
CheckForBigTriangles(std::vector<Stuff::Vector2DScalar>* lightMapUVs2, uint32_t stride)
{
	...
	{
		errorcolour = Stuff::RGBAcolour(0.5f, 0.5f, 0.0f, 1.0f);
		return false;
	}
#if 1
	float sign = -1.0f;
	if (((*lightMapUVs2)[1][0] - (*lightMapUVs2)[0][0]) * ((*lightMapUVs2)[stride - 1][1] - (*lightMapUVs2)[0][1]) > ((*lightMapUVs2)[1][1] - (*lightMapUVs2)[0][1]) * ((*lightMapUVs2)[stride - 1][0] - (*lightMapUVs2)[0][0]))
	{
		sign = 1.0f;
	}
#endif
	for (k0 = 0; k0 < stride; k0++)
	{
		...
		if ((((*lightMapUVs2)[k1][0] * (*lightMapUVs2)[k1][0] - 2.0f * (*lightMapUVs2)[k1][0] * (*lightMapUVs2)[k0][0] + (*lightMapUVs2)[k0][0] * (*lightMapUVs2)[k0][0]) + ((*lightMapUVs2)[k1][1] * (*lightMapUVs2)[k1][1] - 2.0f * (*lightMapUVs2)[k1][1] * (*lightMapUVs2)[k0][1] + (*lightMapUVs2)[k0][1] * (*lightMapUVs2)[k0][1])) < (2.0f * (u1v0 * u1v0 - 2.0f * u1v0 * u0v1 + u0v1 * u0v1)))
		{
#if UV_TEST
			...
			DEBUG_STREAM << endl;
#endif
			errorcolour = Stuff::RGBAcolour(0.0f, 0.5f, 0.0f, 1.0f);
			return false;
		}
	}
	return true;
	...
	case MLRLight::PointLight:
	{
		...
		for (i = 0, j = 0, k = 0; i < len; i++, j += stride)
		{
			...
			if (lm == true || CheckForBigTriangles(lightMapUVs, stride) == true)
			{
				...
				Stuff::RGBAcolour color(sq_falloff, sq_falloff, sq_falloff, 1.0f);
	...
	case MLRLight::SpotLight:
	{
		...
		for (i = 0, j = 0, k = 0; i < len; i++, j += stride)
		{
			...
			for (k = 0; k < stride; k++)
			{
				...
				if ((*lightMapUVs)[k][0] < -1.5f || (*lightMapUVs)[k][0] > 1.5f || (*lightMapUVs)[k][1] < -1.5f && (*lightMapUVs)[k][1] > 1.5f)
				{
					tooBig++;
				}
			}
#if 1
			if (tooBig == 0 && behindCount == 0 && falloffCount < stride && (lm == true || CheckForBigTriangles(lightMapUVs, stride) == true))
			{
			...
			if (tooBig != 0)
			{
				...
				for (k = 0; k < stride; k++)
				{
					lightMap->Addcolour(Stuff::RGBAcolour(0.0f, 0.0f, 0.5f, 1.0f));
				}
				for (k = 0; k < stride; k++)
				{
			...
			else if (behindCount != 0)
			{
				...
				for (k = 0; k < stride; k++)
				{
					lightMap->Addcolour(Stuff::RGBAcolour(0.5f, 0.0f, 0.0f, 1.0f));
				}
				for (k = 0; k < stride; k++)
				{
					lightMap->AddUVs(0.5f, 0.5f);
...
//
MLR_I_PMesh*
MidLevelRenderer::CreateIndexedCube_Nocolour_NoLit(float half, MLRState* state)
{
#ifdef _GAMEOS_HPP_
	gos_PushCurrentHeap(Heap);
#endif
...
//
MLRShape*
MidLevelRenderer::CreateIndexedIcosahedron_Nocolour_NoLit(
