...
#define XAPOASSERT(exp)                                                    \
	if (!(exp))                                                            \
	{                                                                      \
		OutputDebugStringA("XAPO ASSERT: " #exp ", {" __FUNCTION__ "}\n"); \
		__debugbreak();                                                    \
	}
#else
...
#if XAPODEBUG
#define XAPOASSERT_NO_OUTPUT(exp) \
	if (!(exp))                   \
	{                             \
		__debugbreak();           \
	}
...
protected:
	...
	{
		return m_pRegistrationProperties;
	}
	BOOL IsLocked()
	{
		return m_fIsLocked;
...
public:
	...
	(REFIID riid, _Outptr_ void** ppInterface)
	{
	...
	(_Outptr_ XAPO_REGISTRATION_PROPERTIES** ppRegistrationProperties);
	...
	(const WAVEFORMATEX* pOutputFormat, const WAVEFORMATEX* pRequestedInputFormat, _Outptr_opt_ WAVEFORMATEX** ppSupportedInputFormat);

	// Queries if a specific output format is supported for a given input format.
	// This default implementation assumes only the format described by the
	// XAPOBASE_DEFAULT_FORMAT values are supported for both input and output.
	STDMETHOD(IsOutputFormatSupported)
	(const WAVEFORMATEX* pInputFormat, const WAVEFORMATEX* pRequestedOutputFormat, _Outptr_opt_ WAVEFORMATEX** ppSupportedOutputFormat);

	// Performs any effect-specific initialization.
	// This default implementation is a no-op and only returns S_OK.
	STDMETHOD(Initialize)
	(_In_reads_bytes_opt_(DataByteSize) const void*, UINT32 DataByteSize)
	{
...
public:
	...
	// retrieves the requested interface pointer if supported
	STDMETHOD(QueryInterface)
	(REFIID riid, _Outptr_result_maybenull_ void** ppInterface)
	{
		XAPOASSERT(ppInterface != NULL);
		HRESULT hr = S_OK;
	...
	(_In_reads_bytes_(ParameterByteSize) const void* pParameters, UINT32 ParameterByteSize);
	...
	(_Out_writes_bytes_(ParameterByteSize) void* pParameters, UINT32 ParameterByteSize);
