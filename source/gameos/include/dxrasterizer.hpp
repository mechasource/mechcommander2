...
HRESULT __stdcall CheckEnumProc(DDPIXELFORMAT* ddpfpixelformat, PVOID pcontext);
...
enum class __compatibilityconst
{
	...
	DisablecolourControl = 1 << 24, // Disable gamma and color controls
...
extern COLORREF goscolourTable[19];
...
BOOL __stdcall DirectDrawEnumerateCallback(GUID* pguid, PSTR, PSTR, PVOID pcontext);
HRESULT __stdcall DirectDrawEnumDisplayModesCallback(LPDDSURFACEDESC2 pddsd, PVOID pcontext);
