...
public:
	...
	static bool initTerrainFromPCV(std::wstring_view fileName);
	static bool reassignheightsFromTGA(std::wstring_view fileName, int32_t min, int32_t max);

	bool save(std::wstring_view fileName, bool quickSave = false);
	bool quickSave(std::wstring_view fileName);
	static bool saveheightMap(File* file);
	bool saveMissionFitFileStuff(FitIniFile& fitFile);

	static void setMapName(std::wstring_view name);
	static std::wstring_view getMapName()
	{
	...
	void MissionName(std::wstring_view missionName)
	{
	...
	void Author(std::wstring_view author)
	{
		m_author = author;
	}

	std::wstring_view Blurb()
	{
		return m_blurb;
	}
	void Blurb(std::wstring_view blurb)
	{
		m_blurb = blurb;
	}
	...
	std::wstring_view Blurb2()
	{
		return m_blurb2;
	}
	void Blurb2(std::wstring_view blurb2)
	{
		m_blurb2 = blurb2;
	}
	...
	void VideoFilename(std::wstring_view videoFilename)
	{
	...
	void DownloadURL(std::wstring_view downloadURL)
	{
...
private:
	...
	std::wstring_view m_videoFilename;
	int32_t m_numRandomRPbuildings;
	std::wstring_view m_downloadURL;
