...
#endif
{
  public:
	...
	size_t GetLength(void) const { return N; }
	size_t GetSize(void) const { return N * sizeof(T); }
	...
	friend MemoryStream& MemoryStreamIO_Write(
		MemoryStream* stream, const StaticArrayOf<T, N>* array);

	friend MemoryStream& MemoryStreamIO_Read(
		MemoryStream* stream, StaticArrayOf<T, N>* array);
...
template <class T, size_t N> inline StaticArrayOf<T, N>::StaticArrayOf(void) {}

template <class T, size_t N>
inline StaticArrayOf<T, N>::StaticArrayOf(const StaticArrayOf<T, N>& array)
{
	AssignData(array.data, N);
}

template <class T, size_t N>
inline StaticArrayOf<T, N>::StaticArrayOf(const T& value)
{
	AssignValue(value);
}

template <class T, size_t N>
inline StaticArrayOf<T, N>::StaticArrayOf(T* data_to_copy, size_t length)
{
...
template <class T, size_t N>
StaticArrayOf<T, N>& StaticArrayOf<T, N>::operator=(
...
template <class T, size_t N> inline T& StaticArrayOf<T, N>::operator[](size_t i)
{
	Verify(i < N);
	return data[i];
}
...
template <class T, size_t N>
inline const T& StaticArrayOf<T, N>::operator[](size_t i) const
{
	Verify(i < N);
	return data[i];
}
...
template <class T, size_t N>
void StaticArrayOf<T, N>::AssignValue(const T& value)
{
...
template <class T, size_t N>
void StaticArrayOf<T, N>::AssignData(const T* data_to_copy, size_t length)
{
...
template <class T, size_t N>
MemoryStream& MemoryStreamIO_Write(
	MemoryStream* stream, const StaticArrayOf<T, N>* array)
{
	Check_Object(stream);
	Check_Pointer(array);
...
template <class T, size_t N>
MemoryStream& MemoryStreamIO_Read(
	MemoryStream* stream, StaticArrayOf<T, N>* array)
{
	Check_Object(stream);
	Check_Pointer(array);
...
#endif
{
  public:
	...
	size_t GetSize(void) const { return length * sizeof(T); }
	T* GetData(void) { return data; }
	const T* GetData(void) const { return data; }
	...
	friend MemoryStream& MemoryStreamIO_Write<T>(
		MemoryStream* stream, const DynamicArrayOf<T>* array);

	friend MemoryStream& MemoryStreamIO_Read<T>(
		MemoryStream* stream, DynamicArrayOf<T>* array);
...
template <class T> DynamicArrayOf<T>::DynamicArrayOf(void)
{
	data   = nullptr;
	length = 0;
}
...
template <class T> DynamicArrayOf<T>::DynamicArrayOf(size_t length_to_set)
{
	data   = nullptr;
	length = 0;
...
template <class T>
DynamicArrayOf<T>& DynamicArrayOf<T>::operator=(const DynamicArrayOf<T>& array)
{
	if (this != &array)
		CopyArray(array);
	return *this;
...
template <class T> inline T& DynamicArrayOf<T>::operator[](size_t i)
{
	Check_Pointer(data);
	Verify(i < length);
	return data[i];
...
template <class T> inline const T& DynamicArrayOf<T>::operator[](size_t i) const
{
	Verify(i < length);
	return data[i];
}
...
template <class T>
void DynamicArrayOf<T>::AssignValue(const T& value, size_t length_to_set)
{
	SetStorageLength(length_to_set);
	// Do not memcopy, object semantics may be required
	for (size_t i = 0; i < length; i++)
...
template <class T>
void DynamicArrayOf<T>::AssignData(const T* data_to_copy, size_t length_to_set)
{
	Check_Pointer(data_to_copy);
	SetStorageLength(length_to_set);
	// Do not memcopy, object semantics may be required
...
template <class T> void DynamicArrayOf<T>::SetLength(size_t length_to_set)
{
...
template <class T>
void DynamicArrayOf<T>::SetStorageLength(size_t length_to_set)
{
	if (length_to_set != length)
	{
		if (length_to_set > 0)
...
template <class T>
void DynamicArrayOf<T>::CopyArray(const DynamicArrayOf<T>& array)
{
	if (array.length > 0)
	{
