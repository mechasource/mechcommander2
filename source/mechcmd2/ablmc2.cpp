...
int32_t getMoversWithinRadius(MoverPtr* moverList, Stuff::Vector3D center, float radius,
	int32_t teamID, int32_t commanderID, bool getEnemies, bool sortDescending, bool ignoreOrder)
{
...
void execSelectContact(void)
{
	...
	if (CurObject->isMover())
	{
		...
		if (object && object->getTeam() &&
			object->getContactStatus(me->getTeam()->getId(), true) != CONTACT_NONE)
		{
			CurContact = object;
			code	   = NO_ERROR;
		}
...
//*****************************************************************************
void execGetObjectiveSuccess(void) { ABLi_pushBoolean(mission->checkObjectiveSuccess()); }

//*****************************************************************************
void execGetObjectiveFailed(void) { ABLi_pushBoolean(mission->checkObjectiveFailed()); }
...
//*****************************************************************************
void execPlayerInCombat(void) { ABLi_pushBoolean(MechWarrior::anyPlayerInCombat()); }

//*****************************************************************************
void execGetSensorsActive(void) { ABLi_pushBoolean(SensorSystemManager::enemyInLOS); }

//*****************************************************************************
void execGetCurrentMusicId(void) { ABLi_pushInteger(soundSystem->getCurrentMusicId()); }

void execGetMissionTuneId(void) { ABLi_pushInteger(mission->getMissionTuneId()); }
...
void execIsGateOpen(void)
{
	...
	void execCoreCapture(void)
	{
		...
		if (CurWarrior && ((objectID == 0) || obj))
			targetID = CurWarrior->coreCapture((objectID == 0) ? nullptr : obj, params);
	...
	void execRequestHelp(void)
	{
		...
		if (CurWarrior->getTeam())
		{
			...
			int32_t numFriendlies = getMoversWithinRadius(friendlies, friendlyPos, friendlyRadius,
				CurWarrior->getTeam()->getId(), commanderID, false, true, priority > 0);
			...
			if (enemy && enemy->getTeam() && (enemy->getTeam() != CurWarrior->getTeam()))
			{
				...
				int32_t numEnemies = getMoversWithinRadius(
					enemies, enemyPos, enemyRadius, enemy->getTeamId(), -1, false, true, true);
				calcAttackPlan(
	...
	void execRequestTarget(void)
	{
		...
		if (CurWarrior->getTeam())
		{
			...
			if (MPlayer || (CurWarrior->getTeam() == Team::home))
				commanderID = CurWarrior->getCommander()->getId();
			int32_t numFriendlies = getMoversWithinRadius(friendlies, friendlyPos, radius,
				CurWarrior->getTeam()->getId(), commanderID, false, true, true);
			int32_t numEnemies	= getMoversWithinRadius(
				   enemies, friendlyPos, radius, CurWarrior->getTeam()->getId(), -1, true, true, true);
...
#define MAX_SHELTERS 10
	...
	void ablEndlessStateCallback(UserFile * log)
	{
		...
		log->write(s);
		sprintf(
			s, "     NumFunctionalWeapons = %d", CurWarrior->getVehicle()->numFunctionalWeapons);
		log->write(s);
		sprintf(s, "     TacOrder = ");
		...
		if (LastCoreAttackTarget)
		{
			...
			sprintf(s, "Target Position = (%.3f, %.3f, %.3f) [%d, %d]", pos.x, pos.y, pos.z,
				LastCoreAttackTarget->cellPositionRow, LastCoreAttackTarget->cellPositionCol);
			log->write(s);
	...
	void initABL(void)
	{
		...
		ABLi_addFunction("getmissionwon", false, nullptr, "b", execGetMissionWon);
		...
		ABLi_addFunction("getmissiontune", false, nullptr, "i", execGetMissionTuneId);
		...
		ABLi_addFunction("getgeneralalarm", false, nullptr, "i", execGetGeneralAlarm);
		ABLi_addFunction("setgeneralalarm", false, "i", nullptr, execSetGeneralAlarm);
		...
		ABLi_addFunction("logisticsscreenid", false, nullptr, "i", execLogisticsScreenId);
		ABLi_addFunction(
			"logisticsanimationcallout", false, "ibri", "b", execLogisticsAnimationCallout);
		ABLi_addFunction("logisticsincallout", false, nullptr, "b", execLogisticsInCallout);
		ABLi_addFunction("isplayingvoiceover", false, nullptr, "b", execIsPlayingVoiceOver);
		ABLi_addFunction("getlogisticstime", false, nullptr, "r", execGetLogisticsTime);
		ABLi_addFunction("stopvoiceover", false, nullptr, nullptr, execStopVoiceOver);
		ABLi_addFunction("incallout", false, nullptr, "b", execInCallout);
		ABLi_addFunction("setinvulnerable", false, "b", nullptr, execSetInvulnerable);
