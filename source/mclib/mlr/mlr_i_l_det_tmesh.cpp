...
//
void
MLR_I_L_DeT_TMesh::InitializeClass(void)
{
	_ASSERT(!DefaultData);
...
//
void
MLR_I_L_DeT_TMesh::Save(std::iostream stream)
{
	// Check_Object(this);
	Check_Object(stream);
...
//
void
MLR_I_L_DeT_TMesh::SetNormalData(const Stuff::Vector3D* data, size_t dataSize)
{
	// Check_Object(this);
	Check_Pointer(data);
...
//
void
MLR_I_L_DeT_TMesh::GetNormalData(Stuff::Vector3D** data, size_t* dataSize)
{
	// Check_Object(this);
...
//
void
MLR_I_L_DeT_TMesh::SetcolourData(
#if COLOR_AS_DWORD
	const uint32_t* data,
#else
