...
static void setOverlayType(
	int32_t& mask, int32_t direction, Overlays overlayType)
{
	if ((0 <= direction) && (4 > direction))
	{
...
#define CONNECTIVITY_MASK(up, right, down, left)                               \
	((OverlayTypeToInt(up) & 0xff) + ((OverlayTypeToInt(right) & 0xff) << 8) + \
		((OverlayTypeToInt(down) & 0xff) << 16) +                              \
		((OverlayTypeToInt(left) & 0xff) << 24))

static int32_t connectivityMask(
	Overlays up, Overlays right, Overlays down, Overlays left)
{
...
static int32_t connectivityMask()
{
	return connectivityMask(
		INVALID_OVERLAY, INVALID_OVERLAY, INVALID_OVERLAY, INVALID_OVERLAY);
}
	...
	for (i = 0; i < 4; i++)
	{
		...
		if (INVALID_OVERLAY != type)
		{
			if ((0 == overlayTypeList.Count()) ||
				(EList<Overlays, Overlays>::INVALID_ITERATOR ==
					overlayTypeList.Find(type)))
			{
...
#define IOM_DOWN 4
#define IOM_LEFT 8
static int32_t IndexOffsetToConnectivityMaskMapping[15] = {IOM_UP | IOM_DOWN,
	IOM_RIGHT | IOM_LEFT, IOM_UP | IOM_LEFT, IOM_DOWN | IOM_LEFT,
	IOM_RIGHT | IOM_DOWN, IOM_UP | IOM_RIGHT,
	IOM_UP | IOM_RIGHT | IOM_DOWN | IOM_LEFT, IOM_UP | IOM_RIGHT | IOM_LEFT,
	IOM_UP | IOM_DOWN | IOM_LEFT, IOM_RIGHT | IOM_DOWN | IOM_LEFT,
	IOM_UP | IOM_RIGHT | IOM_DOWN, IOM_DOWN, IOM_RIGHT, IOM_UP, IOM_LEFT};

/* Transition overlays */
/* mh: this array was generated by a GWBASIC program called "trnstns.bas"
   (incase they need to be regenerated) */
static int32_t TransitionConnectivityMaskToIndexOffsetMapping[81] = {-1, -1, -1,
	-1, -1, 5, -1, 8, -1, -1, -1, 2, -1, -1, 30, 6, 18, 22, -1, 0, -1, 9, 32,
	13, -1, 24, -1, -1, -1, 11, -1, -1, 17, 3, 35, 27, -1, -1, 34, -1, -1, 37,
	31, 38, 46, 7, 28, 15, 19, 39, 49, 23, 47, 43, -1, 4, -1, 1, 29, 21, -1, 12,
	-1, 10, 16, 26, 33, 36, 45, 14, 48, 42, -1, 20, -1, 25, 44, 41, -1, 40, -1};
...
static int32_t TransitionIndexOffsetToConnectivityMaskMapping[50] = {
	...
	TIOM_LEFT1 + TIOM_UP2, TIOM_UP1 + TIOM_RIGHT2 + TIOM_LEFT2,
	TIOM_RIGHT1 + TIOM_UP2 + TIOM_DOWN2, TIOM_DOWN1 + TIOM_RIGHT2 + TIOM_LEFT2,
	TIOM_LEFT1 + TIOM_UP2 + TIOM_DOWN2, TIOM_UP1 + TIOM_DOWN1 + TIOM_LEFT2,
	TIOM_RIGHT1 + TIOM_LEFT1 + TIOM_UP2, TIOM_UP1 + TIOM_DOWN1 + TIOM_RIGHT2,
...
{
	return ((OverlayTypeToInt(oOverlayType) & 0xffff) << 16) +
		   (indexOffset & 0xffff);
}
	...
	int32_t mask, Overlays currentbrush = INVALID_OVERLAY)
{
	...
	if (!((DIRT_ROAD == tmpType) || (PAVED_ROAD == tmpType) ||
			(TWO_LANE_DIRT_ROAD == tmpType) || (DAMAGED_ROAD == tmpType) ||
			(RUNWAY == tmpType) || (OBRIDGE == tmpType) ||
	...
	if (2 == numOverlayTypes)
	{
		...
		int32_t i			  = 0;
		while ((INVALID_OVERLAY == overlayType1) ||
			   (INVALID_OVERLAY == overlayType2))
		{
			gosASSERT(4 > i);
		...
		if (0 == pOverlaysToTransitionMapping)
		{
			...
			int32_t pair;
			pair = (OverlayTypeToInt(DIRT_ROAD) << 16) +
				   OverlayTypeToInt(PAVED_ROAD);
			pOverlaysToTransitionMapping->insert(
				OTMap::value_type(pair, X_DIRT_PAVED));
			pair = (OverlayTypeToInt(DIRT_ROAD) << 16) +
				   OverlayTypeToInt(TWO_LANE_DIRT_ROAD);
			pOverlaysToTransitionMapping->insert(
				OTMap::value_type(pair, X_DIRT_2LANEDIRT));
			pair = (OverlayTypeToInt(DIRT_ROAD) << 16) +
				   OverlayTypeToInt(DAMAGED_ROAD);
			pOverlaysToTransitionMapping->insert(
				OTMap::value_type(pair, X_DIRT_DAMAGED));
			pair = (OverlayTypeToInt(TWO_LANE_DIRT_ROAD) << 16) +
				   OverlayTypeToInt(PAVED_ROAD);
			pOverlaysToTransitionMapping->insert(
				OTMap::value_type(pair, X_2LANEDIRT_PAVED));
			pair = (OverlayTypeToInt(DAMAGED_ROAD) << 16) +
				   OverlayTypeToInt(PAVED_ROAD);
			pOverlaysToTransitionMapping->insert(
				OTMap::value_type(pair, X_DAMAGED_PAVED));
			pair = (OverlayTypeToInt(TWO_LANE_DIRT_ROAD) << 16) +
				   OverlayTypeToInt(DAMAGED_ROAD);
			pOverlaysToTransitionMapping->insert(
				OTMap::value_type(pair, X_2LANEDIRT_DAMAGED));
			pair = (OverlayTypeToInt(PAVED_ROAD) << 16) +
				   OverlayTypeToInt(OBRIDGE);
			pOverlaysToTransitionMapping->insert(
				OTMap::value_type(pair, X_PAVED_ROAD_BRIDGE));
		}
		OTMap::iterator it;
		int32_t pair;
		pair = (OverlayTypeToInt(overlayType1) << 16) +
			   OverlayTypeToInt(overlayType2);
		...
		else
		{
			...
			int32_t indexOffset =
				TransitionConnectivityMaskToIndexOffsetMapping[transitionMask];
	...
	if (1 == numOverlayTypes)
	{
			...
			(DAMAGED_BRIDGE == tmpType))
		{
			...
			indexOffset =
				ConnectivityMaskToIndexOffsetMapping[nonTransitionMask];
		}
...
static int32_t TextureToConnectivityMaskMapping(int32_t iTexture)
{
		...
		(DAMAGED_BRIDGE == tmpType))
	{
		...
		int32_t nonTransitionMask =
			IndexOffsetToConnectivityMaskMapping[indexOffset];
			 ...
			 (X_PAVED_ROAD_BRIDGE == tmpType))
	{
		if (0 == pTransitionToOverlaysMapping)
		{
			...
			pair = (OverlayTypeToInt(DIRT_ROAD) << 16) +
				   OverlayTypeToInt(PAVED_ROAD);
			pTransitionToOverlaysMapping->insert(
				TOMap::value_type(X_DIRT_PAVED, pair));
			pair = (OverlayTypeToInt(DIRT_ROAD) << 16) +
				   OverlayTypeToInt(TWO_LANE_DIRT_ROAD);
			pTransitionToOverlaysMapping->insert(
				TOMap::value_type(X_DIRT_2LANEDIRT, pair));
			pair = (OverlayTypeToInt(DIRT_ROAD) << 16) +
				   OverlayTypeToInt(DAMAGED_ROAD);
			pTransitionToOverlaysMapping->insert(
				TOMap::value_type(X_DIRT_DAMAGED, pair));
			pair = (OverlayTypeToInt(TWO_LANE_DIRT_ROAD) << 16) +
				   OverlayTypeToInt(PAVED_ROAD);
			pTransitionToOverlaysMapping->insert(
				TOMap::value_type(X_2LANEDIRT_PAVED, pair));
			pair = (OverlayTypeToInt(DAMAGED_ROAD) << 16) +
				   OverlayTypeToInt(PAVED_ROAD);
			pTransitionToOverlaysMapping->insert(
				TOMap::value_type(X_DAMAGED_PAVED, pair));
			pair = (OverlayTypeToInt(TWO_LANE_DIRT_ROAD) << 16) +
				   OverlayTypeToInt(DAMAGED_ROAD);
			pTransitionToOverlaysMapping->insert(
				TOMap::value_type(X_2LANEDIRT_DAMAGED, pair));
			pair = (OverlayTypeToInt(PAVED_ROAD) << 16) +
				   OverlayTypeToInt(OBRIDGE);
			pTransitionToOverlaysMapping->insert(
				TOMap::value_type(X_PAVED_ROAD_BRIDGE, pair));
		}
		TOMap::iterator it;
		...
		int32_t transitionMask =
			TransitionIndexOffsetToConnectivityMaskMapping[indexOffset];
		int32_t mask = connectivityMask();
		int32_t i;
		int32_t j = 1;
		for (i = 0; i < 4; i++)
...
static void boundSafeGetOverlay(
	int32_t tileR, int32_t tileC, Overlays& type, int32_t& Offset)
{
...
static int32_t connectivityMaskBasedOnAdjacentTiles(
	int32_t tileR, int32_t tileC)
{
	...
	if ((DIRT_ROAD == tmpType) || (PAVED_ROAD == tmpType) ||
		(TWO_LANE_DIRT_ROAD == tmpType) || (DAMAGED_ROAD == tmpType) ||
		(RUNWAY == tmpType) || (OBRIDGE == tmpType) ||
		(DAMAGED_BRIDGE == tmpType) || (X_DIRT_PAVED == tmpType) ||
		(X_DIRT_2LANEDIRT == tmpType) || (X_DIRT_DAMAGED == tmpType) ||
		(X_2LANEDIRT_PAVED == tmpType) || (X_DAMAGED_PAVED == tmpType) ||
		(X_2LANEDIRT_DAMAGED == tmpType) || (X_PAVED_ROAD_BRIDGE == tmpType))
	{
	...
	int32_t iConnectivityMask = connectivityMask(
		connectsAbove, connectsToTheRight, connectsBelow, connectsToTheLeft);
	return iConnectivityMask;
}

bool OverlayBrush::paint(
	Stuff::Vector3D& worldPos, int32_t screenX, int32_t screenY)
{
	...
	{
		if ((DIRT_ROAD == type) || (PAVED_ROAD == type) ||
			(TWO_LANE_DIRT_ROAD == type) || (DAMAGED_ROAD == type) ||
			(RUNWAY == type) || (OBRIDGE == type) || (DAMAGED_BRIDGE == type) ||
			(X_DIRT_PAVED == type) || (X_DIRT_2LANEDIRT == type) ||
			(X_DIRT_DAMAGED == type) || (X_2LANEDIRT_PAVED == type) ||
			(X_DAMAGED_PAVED == type) || (X_2LANEDIRT_DAMAGED == type) ||
			(X_PAVED_ROAD_BRIDGE == type))
		{
			...
			if ((lastTileR != tileR) || (lastTileC != tileC))
			{
				...
				if (-1 != brushMotionDirection)
				{
					...
					pAction->addChangedVertexInfo(
						lastTileR, lastTileC); // for undo
					Overlays tmpType;
					int32_t tmpOffset;
					/* update last tile */
					int32_t lastTileConnectivityMask = connectivityMask();
					boundSafeGetOverlay(
						lastTileR, lastTileC, tmpType, tmpOffset);
					int32_t iTexture = texture(tmpType, tmpOffset);
					lastTileConnectivityMask =
						TextureToConnectivityMaskMapping(iTexture);
					int32_t tmpCM = connectivityMaskBasedOnAdjacentTiles(
						lastTileR, lastTileC);
					int32_t i;
					for (i = 0; i < 4; i++)
					{
						if (overlayType(lastTileConnectivityMask, i) !=
							overlayType(tmpCM, i))
						{
							setOverlayType(
								lastTileConnectivityMask, i, INVALID_OVERLAY);
						}
					}
					setOverlayType(
						lastTileConnectivityMask, brushMotionDirection, type);
					int32_t newLastTileTexture =
						ConnectivityMaskToTextureMapping(
							lastTileConnectivityMask, type);
					Overlays newLastTileType =
						overlayTypeFromTexture(newLastTileTexture);
					int32_t newLastTileIndexOffset =
						indexOffsetFromTexture(newLastTileTexture);
					land->setOverlay(lastTileR, lastTileC, newLastTileType,
						newLastTileIndexOffset);
					/* update current tile */
					int32_t tileConnectivityMask =
						connectivityMaskBasedOnAdjacentTiles(tileR, tileC);
					int32_t newTileTexture = ConnectivityMaskToTextureMapping(
						tileConnectivityMask, type);
					Overlays newTileType =
						overlayTypeFromTexture(newTileTexture);
					int32_t newTileIndexOffset =
						indexOffsetFromTexture(newTileTexture);
					land->setOverlay(
						tileR, tileC, newTileType, newTileIndexOffset);
				}
			}
		}
		else
