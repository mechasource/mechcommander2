...
size_t
LinearAllocatorPage::Suballocate(_In_ size_t size, _In_ size_t alignment)
{
...
LinearAllocator::LinearAllocator(
	...
	m_device(pDevice),
	m_pendingPages(nullptr), m_usedPages(nullptr), m_unusedPages(nullptr), m_increment(pageSize), m_numPending(0), m_totalPages(0)
{
...
LinearAllocatorPage*
LinearAllocator::FindPageForAlloc(_In_ size_t size, _In_ size_t alignment)
{
#ifdef _DEBUG
	if (size > m_increment)
...
void
LinearAllocator::FenceCommittedPages(_In_ ID3D12CommandQueue* commandQueue)
{
	// No pending pages
...
void
LinearAllocator::RetirePendingPages()
{
...
LinearAllocatorPage*
LinearAllocator::GetCleanPageForAlloc()
{
...
LinearAllocatorPage*
LinearAllocator::GetPageForAlloc(
	size_t sizeBytes,
	size_t alignment)
{
	// Fast path
...
LinearAllocatorPage*
LinearAllocator::FindPageForAlloc(
...
void
LinearAllocator::UnlinkPage(LinearAllocatorPage* page)
{
	if (page->pPrevPage)
...
void
LinearAllocator::LinkPageChain(LinearAllocatorPage* page, LinearAllocatorPage*& list)
{
#if VALIDATE_LISTS
...
void
LinearAllocator::LinkPage(LinearAllocatorPage* page, LinearAllocatorPage*& list)
{
#if VALIDATE_LISTS
	// Walk the chain and ensure it's not in the list twice
	for (LinearAllocatorPage* cur = list; cur != nullptr; cur = cur->pNextPage)
...
void
LinearAllocator::FreePages(LinearAllocatorPage* page)
{
	while (page != nullptr)
	{
		LinearAllocatorPage* nextPage = page->pNextPage;
...
#if VALIDATE_LISTS
void
LinearAllocator::ValidateList(LinearAllocatorPage* list)
{
...
void
LinearAllocator::ValidatePageLists()
{
...
void
LinearAllocator::SetPageDebugName(LinearAllocatorPage* list)
{
	for (auto page = list; page != nullptr; page = page->pNextPage)
	{
