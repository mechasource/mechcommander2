...
// General DirectX
//
HRESULT __stdcall wCoCreateInstance(
	REFCLSID rclsid, LPUNKNOWN pUnkOuter, uint32_t dwClsContext, REFIID riid, PVOID* ppv);
...
HRESULT __stdcall wLock(
	LPDIRECT3DVERTEXBUFFER7 D3Dvb7, uint32_t dwFlags, PVOID* plpData, puint32_t pdwSize);
...
HRESULT __stdcall wDirectDrawEnumerate(
	LPDDENUMCALLBACK lpCallback, LPDDENUMCALLBACKEX lpCallbackEx, PVOID lpContext);
...
HRESULT __stdcall wBltFast(LPDIRECTDRAWSURFACE7 lpdd7, uint32_t dwX, uint32_t dwY,
	LPDIRECTDRAWSURFACE7 lpDDSrcSurface, LPRECT lpSrcRect, uint32_t dwTrans);
HRESULT __stdcall wFlip(
	LPDIRECTDRAWSURFACE7 lpdd7, LPDIRECTDRAWSURFACE7 lpDDSurfaceTargetOverride, uint32_t dwFlags);
...
HRESULT __stdcall wGetAttachedSurface(LPDIRECTDRAWSURFACE7 lpdds7, LPDDSCAPS2 lpDDSCaps2,
...
HRESULT __stdcall wDeleteAttachedSurface(
	LPDIRECTDRAWSURFACE7 lpdds7, uint32_t dwFlags, LPDIRECTDRAWSURFACE7 lpDDSAttachedSurface);
...
HRESULT __stdcall wCreatePalette(LPDIRECTDRAW7 lpdd7, uint32_t dwFlags, LPPALETTEENTRY lpColorTable,
	LPDIRECTDRAWPALETTE* lplpDDPalette, IUnknown* pUnkOuter);
HRESULT __stdcall wCreateSurface(LPDIRECTDRAW7 lpdd7, LPDDSURFACEDESC2 lpDDSurfaceDesc,
	LPDIRECTDRAWSURFACE7* lplpDDSurface7, IUnknown* pUnkOuter);
...
HRESULT __stdcall wEnumDisplayModes(LPDIRECTDRAW7 lpdd7, uint32_t dwFlags,
	LPDDSURFACEDESC2 lpDDSurfaceDesc, PVOID lpContext, LPDDENUMMODESCALLBACK2 lpEnumModesCallback);
...
HRESULT __stdcall wCreateClipper(
	LPDIRECTDRAW7 lpdd7, uint32_t dwFlags, LPDIRECTDRAWCLIPPER* lplpDDClipper, IUnknown* pUnkOuter);
HRESULT __stdcall wSetDisplayMode(LPDIRECTDRAW7 lpdd7, uint32_t dwWidth, uint32_t dwHeight,
	uint32_t dwBPP, uint32_t dwRefreshRate, uint32_t dwFlags);
HRESULT __stdcall wGetAvailableVidMem(
	LPDIRECTDRAW7 lpdd7, LPDDSCAPS2 lpDDSCaps2, puint32_t lpdwTotal, puint32_t lpdwFree);
...
HRESULT __stdcall wLock(LPDIRECTSOUNDBUFFER lpdsb, uint32_t dwWriteCursor, uint32_t dwWriteBytes,
	PVOID* lplpvAudioPtr1, puint32_t lpdwAudioBytes1, PVOID* lplpvAudioPtr2,
	puint32_t lpdwAudioBytes2, uint32_t dwFlags);
HRESULT __stdcall wUnlock(LPDIRECTSOUNDBUFFER lpdsb, PVOID lplpvAudioPtr1, uint32_t lpdwAudioBytes1,
	PVOID lplpvAudioPtr2, uint32_t lpdwAudioBytes2);
...
HRESULT __stdcall wGetCurrentPosition(
	LPDIRECTSOUNDBUFFER lpdsb, puint32_t lpdwCurrentPlayCursor, puint32_t lpdwCurrentWriteCursor);
HRESULT __stdcall wCreateSoundBuffer(LPDIRECTSOUND lpds, LPCDSBUFFERDESC lpcDSBufferDesc,
...
HRESULT __stdcall wPlay(
	LPDIRECTSOUNDBUFFER lpdsb, uint32_t dwReserved1, uint32_t dwReserved2, uint32_t dwFlags);
...
HRESULT __stdcall wSetOrientation(LPDIRECTSOUND3DLISTENER lpdsb, float fx, float fy, float fz,
	float tx, float ty, float tz, uint32_t flag);
...
HRESULT __stdcall wSet(LPKSPROPERTYSET lpPS, REFGUID rguidPropSet, uint32_t ulId,
	PVOID pInstanceData, uint32_t ulInstanceLength, PVOID pPropertyData, uint32_t ulDataLength);
HRESULT __stdcall wGetFormat(
	LPDIRECTSOUNDBUFFER lpdsb, LPWAVEFORMATEX lpwf, uint32_t sizeIn, puint32_t sizeOut);
...
HRESULT __stdcall wInitialize(PIAMMULTIMEDIASTREAM iammms, STREAM_TYPE StreamType, uint32_t dwFlags,
	PIGRAPHBUILDER pFilterGraph);
HRESULT __stdcall wAddMediaStream(PIAMMULTIMEDIASTREAM iammms, IUnknown* pStreamObject,
	const MSPID* pPurposeID, uint32_t dwFlags, PIMEDIASTREAM* ppNewStream);
HRESULT __stdcall wOpenFile(PIAMMULTIMEDIASTREAM iammms, LPCWSTR pszFileName, uint32_t dwFlags);

HRESULT __stdcall wCreateSample(PIDIRECTDRAWMEDIASTREAM iddms, LPDIRECTDRAWSURFACE pSurface,
	const RECT* pRect, uint32_t dwFlags, IDirectDrawStreamSample** ppSample);
HRESULT __stdcall wGetSurface(
	IDirectDrawStreamSample* iddss, IDirectDrawSurface** ppDirectDrawSurface, RECT* pRect);

HRESULT __stdcall wSetFormat(PIDIRECTDRAWMEDIASTREAM iddss, DDSURFACEDESC* ddsd);
HRESULT __stdcall wGetFormat(PIDIRECTDRAWMEDIASTREAM iddss, DDSURFACEDESC* ddsd_current,
	DDSURFACEDESC* ddsd_desired, puint32_t flags);
HRESULT __stdcall wSetSampleTimes(IStreamSample* iddss, STREAM_TIME* st1, STREAM_TIME* st2);
HRESULT __stdcall wUpdate(
	IStreamSample* iss, uint32_t flags, HANDLE event, PAPCFUNC papcfunc, uint32_t apc);
HRESULT __stdcall wGetFilterGraph(PIAMMULTIMEDIASTREAM iamms, PIGRAPHBUILDER* igb);
HRESULT __stdcall wSetPositions(
	IMediaSeeking* ims, STREAM_TIME* st, uint32_t stflags, STREAM_TIME* end, uint32_t eflags);
HRESULT __stdcall wGetStopPosition(IMediaSeeking* ims, STREAM_TIME* st);
HRESULT __stdcall wGetSampleTimes(
	IDirectDrawStreamSample* iddss, STREAM_TIME* st, STREAM_TIME* end, STREAM_TIME* cur);
...
// Direct Input
//
HRESULT __stdcall wDirectInputCreateEx(
	HINSTANCE hinst, uint32_t dwVersion, REFIID riidltf, PVOID* ppvOut, LPUNKNOWN punkOuter);
HRESULT __stdcall wCreateDeviceEx(LPDIRECTINPUT7 lpdi, REFGUID rguid, REFIID riid,
	PVOID* lplpDirectInputDevice, LPUNKNOWN pUnkOuter);
...
HRESULT __stdcall wGetDeviceData(LPDIRECTINPUTDEVICE7 dev, uint32_t cbObjectData,
	LPDIDEVICEOBJECTDATA rgdod, puint32_t pdwInOut, uint32_t dwFlags);
HRESULT __stdcall wPoll(LPDIRECTINPUTDEVICE7 lpdid2);
HRESULT __stdcall wCreateEffect(LPDIRECTINPUTDEVICE7 lpdid2, REFGUID rguid, LPCDIEFFECT lpeff,
...
HRESULT __stdcall wCancelPriority(
	IDirectPlay4A* dplay4, uint32_t dwMinPriority, uint32_t dwMaxPriority, uint32_t dwFlags);
HRESULT __stdcall wClose(IDirectPlay4A* dplay4);
HRESULT __stdcall wCreateGroup(IDirectPlay4A* dplay4, LPDPID lpidGroup, LPDPNAME lpGroupName,
	PVOID lpData, uint32_t dwDataSize, uint32_t dwFlags);
HRESULT __stdcall wCreateGroupInGroup(IDirectPlay4A* dplay4, DPID idParentGroup, LPDPID lpidGroup,
	LPDPNAME lpGroupName, PVOID lpData, uint32_t dwDataSize, uint32_t dwFlags);
HRESULT __stdcall wCreatePlayer(IDirectPlay4A* dplay4, LPDPID lpidPlayer, LPDPNAME lpPlayerName,
	HANDLE hEvent, PVOID lpData, uint32_t dwDataSize, uint32_t dwFlags);
...
HRESULT __stdcall wEnumConnections(IDirectPlay4A* dplay4, LPCGUID lpguidApplication,
	LPDPENUMCONNECTIONSCALLBACK lpEnumCallback, PVOID lpContext, uint32_t dwFlags);
HRESULT __stdcall wEnumGroupPlayers(IDirectPlay4A* dplay4, DPID idGroup, LPGUID lpguidInstance,
	LPDPENUMPLAYERSCALLBACK2 lpEnumPlayersCallback2, PVOID lpContext, uint32_t dwFlags);
HRESULT __stdcall wEnumGroups(IDirectPlay4A* dplay4, LPGUID lpguidInstance,
	LPDPENUMPLAYERSCALLBACK2 lpEnumPlayersCallback2, PVOID lpContext, uint32_t dwFlags);
HRESULT __stdcall wEnumGroupsInGroup(IDirectPlay4A* dplay4, DPID idGroup, LPGUID lpguidInstance,
	LPDPENUMPLAYERSCALLBACK2 lpEnumCallback, PVOID lpContext, uint32_t dwFlags);
HRESULT __stdcall wEnumPlayers(IDirectPlay4A* dplay4, LPGUID lpguidInstance,
	LPDPENUMPLAYERSCALLBACK2 lpEnumPlayersCallback2, PVOID lpContext, uint32_t dwFlags);
HRESULT __stdcall wEnumSessions(IDirectPlay4A* dplay4, LPDPSESSIONDESC2 lpsd, uint32_t dwTimeout,
	LPDPENUMSESSIONSCALLBACK2 lpEnumSessionsCallback2, PVOID lpContext, uint32_t dwFlags);
HRESULT __stdcall wGetCaps(IDirectPlay4A* dplay4, LPDPCAPS lpDPCaps, uint32_t dwFlags);
HRESULT __stdcall wGetGroupConnectionSettings(
	IDirectPlay4A* dplay4, uint32_t dwFlags, DPID idGroup, PVOID lpData, puint32_t lpdwDataSize);
HRESULT __stdcall wGetGroupData(
	IDirectPlay4A* dplay4, DPID idGroup, PVOID lpData, puint32_t lpdwDataSize, uint32_t dwFlags);
...
HRESULT __stdcall wGetMessageQueue(IDirectPlay4A* dplay4, DPID idFrom, DPID idTo, uint32_t dwFlags,
	puint32_t lpdwNumMsgs, puint32_t lpdwNumBytes);
HRESULT __stdcall wGetPlayerAccount(
	IDirectPlay4A* dplay4, DPID idPlayer, uint32_t dwFlags, PVOID lpData, puint32_t lpdwDataSize);
HRESULT __stdcall wGetPlayerAddress(
	IDirectPlay4A* dplay4, DPID idPlayer, PVOID lpData, puint32_t lpdwDataSize);
HRESULT __stdcall wGetPlayerCaps(
	IDirectPlay4A* dplay4, DPID idPlayer, LPDPCAPS lpPlayerCaps, uint32_t dwFlags);
HRESULT __stdcall wGetPlayerData(
	IDirectPlay4A* dplay4, DPID idPlayer, PVOID lpData, puint32_t lpdwDataSize, uint32_t dwFlags);
...
HRESULT __stdcall wReceive(IDirectPlay4A* dplay4, LPDPID lpidFrom, LPDPID lpidTo, uint32_t dwFlags,
	PVOID lpData, puint32_t lpdwDataSize);
HRESULT __stdcall wSecureOpen(IDirectPlay4A* dplay4, LPCDPSESSIONDESC2 lpsd, uint32_t dwFlags,
	LPCDPSECURITYDESC lpSecurity, LPCDPCREDENTIALS lpCredentials);
HRESULT __stdcall wSendChatMessage(
	IDirectPlay4A* dplay4, DPID idFrom, DPID idTo, uint32_t dwFlags, LPDPCHAT lpChatMessage);
HRESULT __stdcall wSend(IDirectPlay4A* dplay4, DPID idFrom, DPID idTo, uint32_t dwFlags,
	PVOID lpData, uint32_t dwDataSize);
HRESULT __stdcall wSendEx(IDirectPlay4A* dplay4, DPID idFrom, DPID idTo, uint32_t dwFlags,
	PVOID lpData, uint32_t dwDataSize, uint32_t dwPriority, uint32_t dwTimeout, PVOID lpContext,
	puint32_t lpdwMsgID);
HRESULT __stdcall wSetGroupConnectionSettings(
	IDirectPlay4A* dplay4, uint32_t dwFlags, DPID idGroup, LPDPLCONNECTION lpConnection);
HRESULT __stdcall wSetGroupData(
	IDirectPlay4A* dplay4, DPID idGroup, PVOID lpData, uint32_t dwDataSize, uint32_t dwFlags);
HRESULT __stdcall wSetGroupName(
	IDirectPlay4A* dplay4, DPID idGroup, LPDPNAME lpGroupName, uint32_t dwFlags);
HRESULT __stdcall wSetGroupOwner(IDirectPlay4A* dplay4, DPID idGroup, DPID idOwner);
HRESULT __stdcall wSetPlayerData(
	IDirectPlay4A* dplay4, DPID idPlayer, PVOID lpData, uint32_t dwDataSize, uint32_t dwFlags);
HRESULT __stdcall wSetPlayerName(
	IDirectPlay4A* dplay4, DPID idPlayer, LPDPNAME lpPlayerName, uint32_t dwFlags);
