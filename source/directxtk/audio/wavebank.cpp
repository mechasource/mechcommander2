...
public:
	...
	HRESULT Initialize(_In_ AudioEngine* engine, _In_ std::wstring_view wbfilename) noexcept;
...
_Use_decl_annotations_
	HRESULT
	WaveBank::Impl::Initialize(AudioEngine* engine, std::wstring_view wbfilename) noexcept
{
...
// Public constructors.
_Use_decl_annotations_
WaveBank::WaveBank(AudioEngine* engine, std::wstring_view wbfilename)
...
_Use_decl_annotations_ const WAVEFORMATEX* WaveBank::GetFormat(uint32_t index, WAVEFORMATEX* wfx, size_t maxsize) const noexcept
{
	if (index >= pimpl->mReader.Count())
		return nullptr;
...
_Use_decl_annotations_ bool WaveBank::FillSubmitBuffer(uint32_t index, XAUDIO2_BUFFER& buffer, XAUDIO2_BUFFER_WMA& wmaBuffer) const
{
...
_Use_decl_annotations_ void WaveBank::FillSubmitBuffer(uint32_t index, XAUDIO2_BUFFER& buffer) const
{
	memset(&buffer, 0, sizeof(buffer));
...
_Use_decl_annotations_ bool WaveBank::GetPrivateData(uint32_t index, void* data, size_t datasize)
{
	if (index >= pimpl->mReader.Count())
		return false;
