...
__inline HRESULT XAudio2CreateVolumeMeter(__deref_out IUnknown** ppApo, uint32_t /*flags*/ DEFAULT(0))
{
	return CreateAudioVolumeMeter(ppApo);
}

__inline HRESULT XAudio2CreateReverb(__deref_out IUnknown** ppApo, uint32_t /*flags*/ DEFAULT(0))
{
...
typedef struct XAUDIO2FX_VOLUMEMETER_LEVELS
{
	...
	float* pRMSLevels; // Root mean square levels table: receives RMS level for each channel
		...
		// otherwise must have at least ChannelCount elements.
	uint32_t ChannelCount; // Number of channels being processed by the volume meter APO
...
typedef struct XAUDIO2FX_REVERB_PARAMETERS
{
	...
	// Delay times
	uint32_t ReflectionsDelay; // [0, 300] in ms
	uint8_t ReverbDelay; // [0, 85] in ms
	uint8_t RearDelay; // [0, 5] in ms

	// Indexed parameters
	uint8_t PositionLeft; // [0, 30] no units
	uint8_t PositionRight; // [0, 30] no units, ignored when configured to mono
	uint8_t PositionMatrixLeft; // [0, 30] no units
	uint8_t PositionMatrixRight; // [0, 30] no units, ignored when configured to mono
	uint8_t EarlyDiffusion; // [0, 15] no units
	uint8_t LateDiffusion; // [0, 15] no units
	uint8_t LowEQGain; // [0, 12] no units
	uint8_t LowEQCutoff; // [0, 9] no units
	uint8_t HighEQGain; // [0, 8] no units
	uint8_t HighEQCutoff; // [0, 14] no units
...
typedef struct XAUDIO2FX_REVERB_I3DL2_PARAMETERS
{
	...
	int32_t Room; // [-10000, 0] in mB (hundredths of decibels)
	int32_t RoomHF; // [-10000, 0] in mB (hundredths of decibels)
	float RoomRolloffFactor; // [0.0, 10.0]
	float DecayTime; // [0.1, 20.0] in seconds
	float DecayHFRatio; // [0.1, 2.0]
	int32_t Reflections; // [-10000, 1000] in mB (hundredths of decibels)
	float ReflectionsDelay; // [0.0, 0.3] in seconds
	int32_t Reverb; // [-10000, 2000] in mB (hundredths of decibels)
	...
	__out XAUDIO2FX_REVERB_PARAMETERS* pNative)
{
	...
	{
		int32_t index = (int32_t)(-4.0 * log10(pI3DL2->DecayHFRatio));
		if (index < -8)
			index = -8;
