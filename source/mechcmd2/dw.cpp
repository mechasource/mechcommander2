...
// to DW first.  Any problem with running DW OR when DW ends, the GameOS
// exception handler will triggered to give even more data to the user.
//
//--------------------------------------------------------------------------------------------
...
int32_t WINAPI DwExceptionFilter(LPEXCEPTION_POINTERS pep)
{
	...
	//------------------------------------------------------------------------------------------------------------
	// init - Check if we just hit a breakpoint.  If so, continue execution.
	// Debugger will take care of itself.
...
//Explorer\\Registration\\DigitalProductID");
	...
	// Check if we are in fullScreen mode.  If so, switch back to WindowMode to
	// insure DW screen comes up.
	...
	// At this point, call the GameOS exception handler and convert the pep to
	// the data they need!
	ProcessException(pep);
	return 1;
}
