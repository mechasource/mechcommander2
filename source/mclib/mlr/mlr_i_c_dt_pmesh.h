...
public:
	...
	//
protected:
	MLR_I_C_DT_PMesh(ClassData* class_data, std::iostream stream, uint32_t version);
	~MLR_I_C_DT_PMesh(void);

public:
	MLR_I_C_DT_PMesh(ClassData* class_data = MLR_I_C_DT_PMesh::DefaultData);

	static MLR_I_C_DT_PMesh* Make(std::iostream stream, uint32_t version);
...
MLR_I_C_DT_PMesh*
CreateIndexedCube_colour_NoLit_2Tex(
	float, Stuff::RGBAcolour*, MLRState*, MLRState*);
MLRShape*
CreateIndexedIcosahedron_colour_NoLit_2Tex(IcoInfo&, MLRState*, MLRState*);
} // namespace MidLevelRenderer
#endif
