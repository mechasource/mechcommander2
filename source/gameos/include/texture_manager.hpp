...
void __stdcall DecodeBMPDimensions(PCSTR FileName, puint8_t Data,
	uint32_t DataSize, puint32_t pTextureWidth, puint32_t pTextureHeight);
void __stdcall DecodeJPGDimensions(PCSTR FileName, puint8_t Data,
	uint32_t DataSize, puint32_t pTextureWidth, puint32_t pTextureHeight);
#ifdef USEPNG
PVOID __stdcall DecodePNG(PCSTR FileName, puint8_t Data, uint32_t DataSize,
	puint32_t TextureWidth, puint32_t TextureHeight, bool TextureLoad,
	PVOID pDestSurf = nullptr);
void __stdcall DecodePNGDimensions(PCSTR FileName, puint8_t Data,
	uint32_t DataSize, puint32_t pTextureWidth, puint32_t pTextureHeight);
#endif
void DecodeTGADimensions(PCSTR FileName, puint8_t Data, uint32_t DataSize,
	puint32_t pTextureWidth, puint32_t pTextureHeight);

PVOID DecodeBMP(PCSTR FileName, puint8_t Data, uint32_t DataSize,
	puint32_t TextureWidth, puint32_t TextureHeight, bool TextureLoad,
	PVOID pDestSurf = nullptr);
PVOID DecodeJPG(PCSTR FileName, puint8_t Data, uint32_t DataSize,
	puint32_t TextureWidth, puint32_t TextureHeight, bool TextureLoad,
	PVOID pDestSurf = nullptr);
PVOID DecodeTGA(PCSTR FileName, puint8_t Data, uint32_t DataSize,
	puint32_t TextureWidth, puint32_t TextureHeight, bool TextureLoad,
	PVOID pDestSurf = nullptr);
...
extern DDSURFACEDESC2
	TextureDesc[5][2][4]; // arranged by: [Format][want32bpp][compression]
extern int32_t TextureDescScore[5][2]
							   [4]; // validity of current entry in TextrueDesc
...
cuint16_t tFlag_InAGP =
	2; // Allocation is in AGP ( tFlag_InVidMem set separately )
cuint16_t tFlag_Valid = 4; // This CTexInfo has been allocated
cuint16_t tFlag_Alpha =
	8; // Texture format has ALPHA (either keyed or full channel)
cuint16_t tFlag_InSysMem =
	16; // Texture has been converted into system memory format
cuint16_t tFlag_LockRW = 32; // Set when a texture is locked for writing
cuint16_t tFlag_Preload =
	64; // Preload this texture at the end of the current frame
...
cuint16_t tFlag_Special =
	8192; // Special texture (font, chess, or color mipmap)
...
class CTexInfo
{
  private:
	...
	gos_RebuildFunction
		m_pRebuild;			   // Application function to repopulate surface
	...
	uint32_t
		m_nOriginalBytes;	// Size of m_pOriginalSurf (even if not allocated)
	...
	static uint32_t
		FrameNo; // Current Frame number (incremented by PreloadTextures())
	...
	static int32_t
		NumVidTextures; // The number of textures currently in video+AGP
	static CTexInfo MostRecentOriginal; // A recently freed "original" - ony
										// m_pOriginalSurf is valid
	static CTexInfo MostRecentSysMem;   // A recently freed "sysmem" - ony
										// m_pSysMemSurf is valid
  ...
  public:
	...
	{
		return &TextureDesc[Format()][(m_Hints & gosHint_Try32bpp) ? 1 : 0]
						   [(m_Hints &
								(gosHint_Compress0 | gosHint_Compress1)) /
							   gosHint_Compress0];
	}
	static bool ManagerInitialized(void) { return Initialized; }
	bool ValidTexture(void)
	{
		return (CTexInfo::Initialized) && (this >= TexInfo) &&
			   (this - MaximumTextures < TexInfo) && (m_Flags & tFlag_Valid);
	}
	...
	static void PreloadTextures(
		void); // Once per frame, upload any "preload" textures to VRAM/AGP
	static bool RecreateHeaps(
		void); // initializes texture types, ReleaseTextureHeaps()
	static void ReleaseTextures(
		bool ReleaseSysmem); // releases vram and optionally sysmem copies

	// Texture Create.cpp
	// Fundamental new/init/delete of a CtexInfo
	static CTexInfo* Allocate(void);
	void Initialize(gos_TextureFormat Format, PCSTR FileName, puint8_t pBitmap,
		uint32_t Size, uint16_t Width, uint16_t Height, uint32_t Hints,
		gos_RebuildFunction pFunc, PVOID pInstance);
	void Free(void);

	// Texture Update.cpp
	// Rebuild / Reload / Convert / Upload as required to get a usable texture
	// handle
	uint32_t GetD3DTextureHandle(
		void); // HW Rasterizer: VidMemSurf, SW Rasterizer: SysMemSurf
	// Altering the surface contents of a texture
	void Lock(uint32_t MipMapWidth, bool ReadOnly, TEXTUREPTR* TextureInfo);
	void Unlock(void);
	void InvalidateVidMem(
		void); // mark vidmem as invalid and check if current texture
	void UpdateRect(uint32_t DestLeft, uint32_t DestTop, uint32_t* Source,
		uint32_t SourcePitch, uint32_t Width, uint32_t Height);
	// Various setters
	...
	// Original surface
  public:
	LPDIRECTDRAWSURFACE7 GetOriginalSurf(
		bool populate, uint16_t MipMapWidth = 0);

  private:
	void AllocateOriginal(void); // Allocates original 32bpp surface
	void PopulateOriginal(
		void); // Rebuilds or Reloads (assumes surface exists but is not valid)
	void Reload(puint8_t pData, uint32_t Size,
		bool Detect); // reloads 32bpp surface from file or pData
	void Rebuild(
		void); // Calls rebuild function of app to re-populate the surface
	void FreeOriginal(void); // Frees the original surface (assumes it exists
							 // and is ok to free)
	void DecodeImageLevel(PCSTR FileName, puint8_t pSourceData, uint32_t Size,
  ...
  private:
	void AllocateSysMem(
		void); // Allocates (probably 16bpp) system memory surface
	void PopulateSysMem(
		void); // (Assumes valid original) Converts the Original Level(s)
	...
	void GenerateMipMaps(
		void); // Filters top level of SysMem chain to all lower levels
	// Texture VidMem.cpp
	// Video Memory or AGP Surface
	LPDIRECTDRAWSURFACE7 GetVidMemSurf(bool populate, uint16_t MipMapWidth = 0);
	bool CreateTexture(
		void); // Called by AllocateVidMem(void) to attempt texture creation
	void AllocateVidMem(
		void); // Uses various techniques to allocate video memory
  ...
  public:
	...
	static void NewLocalLimit(
		void); // called by debugger when local limit changes
	static void FreeDummyTextures(
		void); // Free the artificially created vidmem textures
  ...
  private:
	...
	static void PurgeTextures(
		void); // removes all app textures and does flip's to let them go
  ...
  public:
	static PSTR GetTextureEvent(int32_t offset)
	{
		return (offset >= LogCount)
				   ? (nullptr)
				   : (LogString[(LogNext + MaximumTextureLogs - 1 - offset) %
								MaximumTextureLogs]);
	}
...
extern uint32_t
	VidMemUsed; // Number of textures actually allocated in video memory
