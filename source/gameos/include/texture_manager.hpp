...
public:
	...
	{
		return (MipmapDisabled(void)) ? 1 : MipLevelsRequired(m_nWidth, m_nHeight);
	}
	PCSTR 					Name(void)
	{
		return m_pName;
	...
	{
		return &TextureDesc[Format(void)][(m_Hints & gosHint_Try32bpp) ? 1 : 0][(m_Hints & (gosHint_Compress0 | gosHint_Compress1)) / gosHint_Compress0];
	}
...
{
	CTexInfo::InitializeTextureManager(void);
}
inline void __stdcall DestroyTextureManager(void)
{
	CTexInfo::DestroyTextureManager(void);
}
