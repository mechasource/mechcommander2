...
bool
FormatIsUAVCompatible(_In_ ID3D12Device* device, bool typedUAVLoadAdditionalFormats, DXGI_FORMAT format)
{
	switch (format)
	{
...
bool
FormatIsBGR(DXGI_FORMAT format)
{
	switch (format)
	{
...
bool
FormatIsSRGB(DXGI_FORMAT format)
{
	switch (format)
	{
...
DXGI_FORMAT
ConvertSRVtoResourceFormat(DXGI_FORMAT format)
{
	switch (format)
	{
	case DXGI_FORMAT_R32G32B32A32_FLOAT:
...
private:
		...
		_In_ ID3D12Device* device)
	{
		D3D12_ROOT_SIGNATURE_FLAGS rootSignatureFlags =
			D3D12_ROOT_SIGNATURE_FLAG_DENY_VERTEX_SHADER_ROOT_ACCESS | D3D12_ROOT_SIGNATURE_FLAG_DENY_DOMAIN_SHADER_ROOT_ACCESS | D3D12_ROOT_SIGNATURE_FLAG_DENY_GEOMETRY_SHADER_ROOT_ACCESS | D3D12_ROOT_SIGNATURE_FLAG_DENY_HULL_SHADER_ROOT_ACCESS | D3D12_ROOT_SIGNATURE_FLAG_DENY_PIXEL_SHADER_ROOT_ACCESS;
...
public:
	Impl(
		_In_ ID3D12Device* device) :
		mDevice(device),
		mInBeginEndBlock(false), mTypedUAVLoadAdditionalFormats(false), mStandardSwizzle64KBSupported(false)
	{
		_ASSERT(device != nullptr);
		...
		_In_ ID3D12CommandQueue* commandQueue)
	{
		...
		HANDLE gpuCompletedEvent = ::CreateEventExW(nullptr, nullptr, 0, EVENT_ALL_ACCESS);
		...
		_ASSERT(mTrackedObjects.empty());
		_ASSERT(mTrackedMemoryResources.empty());
...
private:
	...
	{
		const auto desc = resource->GetDesc();
		_ASSERT(!FormatIsBGR(desc.Format) && !FormatIsSRGB(desc.Format));
		...
		_In_ ID3D12Resource* resource)
	{
		...
		_ASSERT(!FormatIsBGR(resourceDesc.Format) || FormatIsSRGB(resourceDesc.Format));
	...
	{
		const auto resourceDesc = resource->GetDesc();
		_ASSERT(FormatIsBGR(resourceDesc.Format));
...
void
ResourceUploadBatch::GenerateMips(_In_ ID3D12Resource* resource)
{
...
std::future<void>
ResourceUploadBatch::End(_In_ ID3D12CommandQueue* commandQueue)
{
