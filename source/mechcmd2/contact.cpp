...
void ContactInfo::operator delete(PVOID us) { missionHeap->Free(us); }
...
void SensorSystem::operator delete(PVOID us) { missionHeap->Free(us); }
...
void SensorSystem::setOwner(GameObjectPtr newOwner) { owner = newOwner; }
...
void SensorSystem::setShutdown(bool setting)
{
	if (setting)
	{
		if (notShutdown)
		{
			...
			{
				MoverPtr mover =
					(MoverPtr)ObjectManager->get(contacts[i] & 0x7FFF);
...
bool SensorSystem::enabled(void)
{
	if (master && (masterIndex > -1))
	{
		...
		if (owner->isMover())
		{
			...
			return ((mover->sensor != 255) &&
					!mover->inventory[mover->sensor].disabled);
		}
		return (true);
	}
	return (false);
...
//---------------------------------------------------------------------------
void SensorSystem::setRange(float newRange) { range = newRange; }
...
float SensorSystem::getEffectiveRange(void) { return (range * ecmEffect); }
...
int32_t SensorSystem::calcContactStatus(MoverPtr mover)
{
	...
	if (owner->isMover())
	{
		MoverPtr mover = (MoverPtr)owner;
		if ((mover->sensor == 255) ||
			mover->inventory[mover->sensor].disabled || broken)
		{
			return (CONTACT_NONE);
		}
	}
	...
	{
		bool moverNotContact =
			mover->hasNullSignature() || (mover->getEcmRange() != 0.0f);
		...
		if (!moverInvisible && !moverNotContact)
		{
			...
			if (distanceToMover <= sensorRange)
			{
				...
				if ((ecmEffect < 1.0) &&
					(newContactStatus > CONTACT_SENSOR_QUALITY_1))
					newContactStatus--;
				//---------------------------------------------------
		...
		{
			// Target is shutdown, can ONLY be visual cause this platform has no
			// probe.
			float startRadius = 0.0f;
			if (!owner->isMover())
				startRadius = owner->getAppearRadius();
	...
	if (mover->getTeam() && (owner->getTeam() == Team::home) &&
		mover->getTeam()->isEnemy(Team::home) &&
...
int32_t SensorSystem::getSensorQuality(void)
{
	if (owner && owner->getPilot())
	{
		if (owner->getPilot()->isSensorProfileSpecialist())
		{
			...
			{
				return (
					owner->getPilot()->getRank() + CONTACT_SENSOR_QUALITY_1);
			}
...
void SensorSystem::addContact(MoverPtr mover, bool visual)
{
	...
	if (numContacts < MAX_CONTACTS_PER_SENSOR)
	{
		...
		if (notShutdown)
			master->addContact(this, mover, numContacts,
				visual ? CONTACT_VISUAL : getSensorQuality());
		numContacts++;
	}
}
...
void SensorSystem::modifyContact(MoverPtr mover, bool visual)
{
	...
	if (notShutdown)
		master->modifyContact(
			this, mover, visual ? CONTACT_VISUAL : getSensorQuality());
}
...
void SensorSystem::removeContact(int32_t contactIndex)
{
	...
	MoverPtr contact =
		(MoverPtr)ObjectManager->get(contacts[contactIndex] & 0x7FFF);
	...
	if ((numContacts > 0) && (contactIndex != numContacts))
	{
		...
		MoverPtr contact =
			(MoverPtr)ObjectManager->get(contacts[numContacts] & 0x7FFF);
...
void SensorSystem::updateScan(bool forceUpdate)
{
	...
	if (1 /*(nextScanUpdate < scenarioTime) || forceUpdate*/)
	{
		...
		if ((currentScan = scanBattlefield()) >
			-1) // No returns size of largest contact.
		{
			if (owner->isMover() && (owner->getTeam() == Team::home) &&
...
//---------------------------------------------------------------------------
__inline void getLargest(
	int32_t& currentLargest, MoverPtr mover, int32_t contactStatus)
{
	...
	for (size_t i = 0; i < numMovers; i++)
	{
		...
		if (mover->getExists() && (mover->getTeamId() != owner->getTeamId()))
		{
			...
			if (isContact(mover))
			{
				...
				{
					modifyContact(
						mover, contactStatus == CONTACT_VISUAL ? true : false);
...
int32_t SensorSystem::getTeamContacts(
...
void TeamSensorSystem::operator delete(PVOID us) { missionHeap->Free(us); }
...
//---------------------------------------------------------------------------
void TeamSensorSystem::destroy(void) { init(); }
...
{
	static bool isSensor[NUM_CONTACT_STATUSES] = {
...
bool TeamSensorSystem::meetsCriteria(
	GameObjectPtr looker, MoverPtr mover, int32_t contactCriteria)
{
	bool isSensor[NUM_CONTACT_STATUSES] = {
		false, true, true, true, true, false};
...
int32_t TeamSensorSystem::getContacts(GameObjectPtr looker,
	int32_t* contactList, int32_t contactCriteria, int32_t sortType)
{
	...
	for (size_t i = 0; i < numContacts; i++)
	{
		...
		case CONTACT_SORT_DISTANCE:
			sortValues[numValidContacts] =
				looker->distanceFrom(mover->getPosition());
			break;
		}
		numValidContacts++;
	}
...
int32_t TeamSensorSystem::getContactStatus(MoverPtr mover, bool includingAllies)
{
	if (mover->getFlag(OBJECT_FLAG_REMOVED))
		return (CONTACT_NONE);
	return (mover->getContactInfo()->getContactStatus(
		Team::teams[teamId]->getId(), true));
}
...
void TeamSensorSystem::addContact(SensorSystemPtr sensor, MoverPtr contact,
...
SensorSystemPtr TeamSensorSystem::findBestSpotter(
	MoverPtr contact, int32_t* status)
{
	ContactInfoPtr contactInfo = contact->getContactInfo();
	if (!contactInfo)
	{
		...
		sprintf(s,
			...
			contact->getObjectClass(), contact->getPartId(),
			contact->getTeamId());
		STOP((s));
	}
...
void TeamSensorSystem::removeContact(SensorSystemPtr sensor, MoverPtr contact)
{
	...
	if (contactInfo->contactCount[teamId] == 0)
	{
		...
		MoverPtr removedContact =
			(MoverPtr)ObjectManager->get(contacts[contactIndex]);
		...
		if ((numContacts > 0) && (contactIndex != numContacts))
		{
			...
			contacts[contactIndex] = contacts[numContacts];
			MoverPtr mover =
				(MoverPtr)ObjectManager->get(contacts[numContacts]);
	...
	else if (contactInfo->teamSpotter[teamId] == sensor->owner->getHandle())
	{
		...
		if (bestSensor)
		{
			...
			contact->contactInfo->teamSpotter[teamId] =
				bestSensor->owner->getHandle();
		}
	}
}
...
void SensorSystemManager::operator delete(PVOID us) { missionHeap->Free(us); }
...
int32_t SensorSystemManager::init(bool debug)
{
	...
	sensorPool = (SensorSystemPtr*)missionHeap->Malloc(
		MAX_SENSORS * sizeof(SensorSystemPtr));
	...
	Assert(!debug || (Team::numTeams > 0), 0,
		" SensorSystemManager.init: 0 teams ");
...
void SensorSystemManager::removeTeamSensor(
	int32_t teamId, SensorSystemPtr sensor)
{
	if (teamId == -1)
		return;
...
float SensorSystemManager::getEcmEffect(GameObjectPtr victim)
{
	...
	for (size_t i = 0; i < numEcms; i++)
		if (!victim->isFriendly(ecms[i].owner))
			if (ecms[i].owner->distanceFrom(victim->getPosition()) <=
	ecms[i].range) return(0.5);
	*/
	return (1.0);
}
