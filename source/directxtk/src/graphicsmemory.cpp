...
inline size_t
GetPageSizeFromPoolIndex(size_t x)
{
...
void
GraphicsMemory::Commit(_In_ ID3D12CommandQueue* commandQueue)
{
...
#if defined(_XBOX_ONE) && defined(_TITLE)
GraphicsMemory&
GraphicsMemory::Get(_In_opt_ ID3D12Device*)
{
...
#else
GraphicsMemory&
GraphicsMemory::Get(_In_opt_ ID3D12Device* device)
{
...
GraphicsResource::GraphicsResource() noexcept :
	mPage(nullptr), mGpuAddress{}, mResource(nullptr), mMemory(nullptr), mBufferOffset(0), mSize(0)
{
}
...
void
GraphicsResource::Reset(GraphicsResource&& alloc)
{
	if (mPage)
	{
		mPage->Release();
...
SharedGraphicsResource&&
SharedGraphicsResource::operator=(SharedGraphicsResource&& resource) noexcept
{
	mSharedResource = std::move(resource.mSharedResource);
	return std::move(*this);
}
...
SharedGraphicsResource&&
SharedGraphicsResource::operator=(GraphicsResource&& resource)
{
	mSharedResource = std::make_shared<GraphicsResource>(std::move(resource));
	return std::move(*this);
}
...
SharedGraphicsResource&
SharedGraphicsResource::operator=(const SharedGraphicsResource& resource)
{
	mSharedResource = resource.mSharedResource;
	return *this;
}
...
void
SharedGraphicsResource::Reset()
{
	mSharedResource.reset();
}

void
SharedGraphicsResource::Reset(GraphicsResource&& resource)
{
	mSharedResource = std::make_shared<GraphicsResource>(std::move(resource));
}

void
SharedGraphicsResource::Reset(SharedGraphicsResource&& resource)
{
	mSharedResource = std::move(resource.mSharedResource);
}

void
SharedGraphicsResource::Reset(const SharedGraphicsResource& resource)
{
	mSharedResource = resource.mSharedResource;
}
