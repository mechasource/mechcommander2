...
//###########################################################################
//
TreeNode::TreeNode(Tree* tree, Plug* plug) : Link(tree, plug)
{
	less	= nullptr;
	greater = nullptr;
	parent  = nullptr;
...
//###########################################################################
//
void TreeNode::TestInstance(void)
{
	Link::TestInstance();
	if (less != nullptr)
	{
...
//###########################################################################
//
void TreeNode::SetupTreeLinks(
	TreeNode* less, TreeNode* greater, TreeNode* parent)
{
	// Check_Object(this);
	this->less	= less;
...
//###########################################################################
//
void Tree::AddValueImplementation(Plug* plug, PCVOID value)
{
	// Check_Object(this);
	Check_Object(plug);
	/*
...
//###########################################################################
//
Plug* Tree::FindImplementation(PCVOID value)
{
	// Check_Object(this);
	TreeNode* node;
...
//###########################################################################
//
TreeNode* Tree::MakeTreeNode(Plug*, PCVOID)
{
	// Check_Object(this);
...
//###########################################################################
//
int32_t Tree::CompareTreeNodes(TreeNode*, TreeNode*)
{
	// Check_Object(this);
...
//###########################################################################
//
int32_t Tree::CompareValueToTreeNode(PCVOID, TreeNode*)
{
	// Check_Object(this);
...
//###########################################################################
//
TreeNode* Tree::SearchForValue(PCVOID value)
{
	// Check_Object(this);
	TreeNode* node;
	int32_t ret;
...
//###########################################################################
//
TreeIterator::TreeIterator(Tree* tree) : SortedIterator(tree) { First(); }
...
//###########################################################################
//
Plug* TreeIterator::FindImplementation(PCVOID value)
{
	// Check_Object(this);
	TreeNode* node;
...
//###########################################################################
//
void TreeIterator::ReceiveMemo(IteratorMemo memo, PVOID content)
{
	// Check_Object(this);
	if (memo == PlugRemoved)
	{
