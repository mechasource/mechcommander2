...
#define IOM_DOWN 4
#define IOM_LEFT 8
static int32_t IndexOffsetToConnectivityMaskMapping[15] =
{
	IOM_UP | IOM_DOWN,
	IOM_RIGHT | IOM_LEFT,
	IOM_UP | IOM_LEFT,
...
static int32_t TransitionConnectivityMaskToIndexOffsetMapping[81] =
...
int32_t indexOffsetFromTexture(int32_t iTexture) {
	return (iTexture & 0xffff);
}

static int32_t ConnectivityMaskToTextureMapping(int32_t mask, Overlays currentbrush = INVALID_OVERLAY) {
...
static int32_t connectivityMaskBasedOnAdjacentTiles( int32_t tileR, int32_t tileC ) {
	...
	int32_t iConnectivityMask = connectivityMask(connectsAbove, connectsToTheRight, connectsBelow, connectsToTheLeft);
		...
		if ((DIRT_ROAD == type) || (PAVED_ROAD == type) || (TWO_LANE_DIRT_ROAD == type) || (DAMAGED_ROAD == type) || (RUNWAY == type) || (OBRIDGE == type) || (DAMAGED_BRIDGE == type)
			...
			if ((lastTileR != tileR) || (lastTileC != tileC)) {
				...
				if (-1 != brushMotionDirection) {
					...
					int32_t newLastTileTexture = ConnectivityMaskToTextureMapping(lastTileConnectivityMask, type);
					...
					/* update current tile */
					int32_t tileConnectivityMask = connectivityMaskBasedOnAdjacentTiles( tileR, tileC );
					int32_t newTileTexture = ConnectivityMaskToTextureMapping(tileConnectivityMask, type);
