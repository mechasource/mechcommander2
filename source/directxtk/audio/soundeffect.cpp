...
_Use_decl_annotations_
	HRESULT
	SoundEffect::Impl::Initialize(AudioEngine* engine, std::unique_ptr<uint8_t[]>& wavData,
...
void
SoundEffect::Play(float volume, float pitch, float pan)
{
...
void
SoundEffect::UnregisterInstance(_In_ SoundEffectInstance* instance)
{
...
bool
SoundEffect::FillSubmitBuffer(_Out_ XAUDIO2_BUFFER& buffer, _Out_ XAUDIO2_BUFFER_WMA& wmaBuffer) const
{
...
void
SoundEffect::FillSubmitBuffer(_Out_ XAUDIO2_BUFFER& buffer) const
{
	memset(&buffer, 0, sizeof(buffer));
	buffer.AudioBytes = pImpl->mAudioBytes;
	buffer.pAudioData = pImpl->mStartAudio;
