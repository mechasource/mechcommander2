...
namespace Stuff {
	...
	public:
		...
		Matrix4D& operator=(const YawPitchRoll &angles);
		Matrix4D& operator=(const UnitQuaternion &q);
		Matrix4D& operator=(const Point3D &p);

		Matrix4D& BuildRotation(const EulerAngles &angles);
		Matrix4D& BuildRotation(const YawPitchRoll &angles);
	...
	//vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
		...
		//
		void SetPerspective(float near_clip, float far_clip, float left_clip, float right_clip, float top_clip, float bottom_clip);
		void GetPerspective(float *near_clip, float *far_clip, float *left_clip, float *right_clip, float *top_clip, float *bottom_clip) const;

		void SetPerspective(float near_clip, float far_clip, const Radian &horizontal_fov, float height_to_width);
		void GetPerspective(float *near_clip, float *far_clip, Radian *horizontal_fov, float *height_to_width) const;
	};
}
#endif
