...
void
SoundEffectInstance::Impl::Play(bool loop)
{
	if (!mBase.voice)
	{
		if (mWaveBank)
...
// Public methods.
void
SoundEffectInstance::Play(bool loop)
{
	pImpl->Play(loop);
}

void
SoundEffectInstance::Stop(bool immediate)
{
...
void
SoundEffectInstance::SetVolume(float volume)
{
	pImpl->mBase.SetVolume(volume);
}

void
SoundEffectInstance::SetPitch(float pitch)
{
	pImpl->mBase.SetPitch(pitch);
}

void
SoundEffectInstance::SetPan(float pan)
{
	pImpl->mBase.SetPan(pan);
}

void
SoundEffectInstance::Apply3D(const AudioListener& listener, const AudioEmitter& emitter, bool rhcoords)
{
...
// Notifications.
void
SoundEffectInstance::OnDestroyParent()
{
