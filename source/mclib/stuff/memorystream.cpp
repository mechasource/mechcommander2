...
MemoryStream::ClassData* MemoryStream::DefaultData = nullptr;

//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
void MemoryStream::InitializeClass()
{
	_ASSERT(!DefaultData);
	DefaultData =
		new ClassData(MemoryStreamClassID, "Stuff::MemoryStream", RegisteredClass::DefaultData);
	Check_Object(DefaultData);
}

//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
void MemoryStream::TerminateClass()
{
	Check_Object(DefaultData);
	delete DefaultData;
	DefaultData = nullptr;
...
MemoryStream::MemoryStream(PVOID stream_start, size_t stream_size, size_t initial_offset)
	: RegisteredClass(DefaultData)
{
	// Check_Pointer(this);
	streamStart = static_cast<puint8_t>(stream_start);
	streamSize  = stream_size;
	_ASSERT(initial_offset <= stream_size);
...
MemoryStream::MemoryStream(
	ClassData* class_data, PVOID stream_start, size_t stream_size, size_t initial_offset)
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
MemoryStream& MemoryStream::ReadBytes(PVOID ptr, size_t number_of_bytes)
{
	// Check_Object(this);
	Check_Pointer(ptr);
	_ASSERT(number_of_bytes > 0);
#if defined(_ARMOR)
	size_t remaining = GetBytesRemaining();
	_ASSERT(remaining >= number_of_bytes);
#endif
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
bool MemoryStream::ReadLine(PSTR buffer, size_t max_number_of_bytes, char continuator)
{
	// Check_Object(this);
	Check_Pointer(buffer);
	_ASSERT(max_number_of_bytes > 0);
	(void)continuator;
	//
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
Stuff::MString MemoryStream::ReadString(size_t size_of_buffer, char continuator)
{
	// Check_Object(this);
	_ASSERT(size_of_buffer > 0);
	static char buffer[1024];
	_ASSERT(size_of_buffer < sizeof(buffer));
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
MemoryStream& MemoryStream::ReadSwappedBytes(PVOID ptr, size_t number_of_bytes)
{
	// Check_Object(this);
	Check_Pointer(ptr);
	_ASSERT(number_of_bytes > 0);
	_ASSERT(GetBytesRemaining() >= number_of_bytes);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
void MemoryStream::WriteLine(PSTR buffer)
{
	// Check_Object(this);
	Check_Pointer(buffer);
...
//
#if _CONSIDERED_DISABLED
MemoryStream& MemoryStream::WriteBytes(PCVOID ptr, size_t number_of_bytes)
{
	// Check_Object(this);
	Check_Pointer(ptr);
	_ASSERT(number_of_bytes > 0);
...
MemoryStream& MemoryStream::WriteSwappedBytes(PCVOID ptr, size_t number_of_bytes)
{
	// Check_Object(this);
	Check_Pointer(ptr);
	_ASSERT(number_of_bytes > 0);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
MemoryStream& MemoryStreamIO::Write(MemoryStream* stream, const MemoryStream* input_stream)
{
	Check_Object(stream);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
MemoryStream& MemoryStream::ReadBit(bool& bit_value)
{
	// Check_Object(this);
	int32_t result = 0;
	...
	_ASSERT(currentBit >= 0);
	_ASSERT(currentBit < 8);
...
MemoryStream& MemoryStream::WriteBit(const bool& bit_value)
{
	// Check_Object(this);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
MemoryStream& MemoryStream::ReadUnsafeBits(PVOID ptr, uint32_t number_of_bits)
{
	// Check_Object(this);
	Check_Pointer(ptr);
	if (currentBit == Empty_Bit_Buffer)
		...
		*dest &= bit_mask;
		...
		if (currentBit == 8)
		{
			...
			if (bits_in_second_source_byte)
			{
				...
				// mask the used bits out of the buffer and then shift the
				// buffer to the correct bit location.  Then combine it with the
				// destination
...
MemoryStream& MemoryStream::WriteBits(PCVOID ptr, uint32_t number_of_bits)
{
	// Check_Object(this);
	Check_Pointer(ptr);
	if (currentBit == Empty_Bit_Buffer)
	...
	while (bits_remaining > 0)
	{
		...
		if (currentBit == 8)
		{
			int32_t bits_in_second_destination_byte =
				total_bits_to_be_written - bits_in_first_destination_byte;
			// SPEW(("jerryeds", "WRITE : %x", workingBitBuffer));
			WriteBytes(&workingBitBuffer, 1);
			++source_byte_counter;
			workingBitBuffer = 0x00;
...
MemoryStream& MemoryStream::ReadBitsToScaledInt(
...
MemoryStream& MemoryStream::WriteScaledIntToBits(
...
MemoryStream& MemoryStream::ReadBitsToScaledFloat(
...
MemoryStream& MemoryStream::WriteScaledFloatToBits(
...
void MemoryStream::TestInstance(void) const
{
	...
	_ASSERT(size_t(currentPosition - streamStart) <= streamSize);
}
...
//
static inline size_t Calculate_Buffer_Size(size_t needed) { return (needed + 0x80) & ~0x7F; }

//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
DynamicMemoryStream::DynamicMemoryStream(size_t stream_size) : MemoryStream(nullptr, stream_size)
{
	// Check_Pointer(this);
...
DynamicMemoryStream::DynamicMemoryStream(
	PVOID stream_start, size_t stream_size, size_t initial_offset)
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
MemoryStream& DynamicMemoryStream::WriteBytes(PCVOID ptr, size_t number_of_bytes)
{
	// Check_Object(this);
	Check_Pointer(ptr);
	_ASSERT(number_of_bytes > 0);
	...
	Mem_Copy(GetPointer(), ptr, number_of_bytes, MemoryStream::GetBytesRemaining());
	AdvancePointer(number_of_bytes);
