...
public:
	...
	SpriteFont(ID3D12Device* device, ResourceUploadBatch& upload, _In_ std::wstring_view filename, D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptorDest, D3D12_GPU_DESCRIPTOR_HANDLE gpuDescriptor, bool forceSRGB = false);
	...
	void XM_CALLCONV DrawString(_In_ SpriteBatch* spriteBatch, _In_ std::wstring_view text, DirectX::XMFLOAT2 const& position, DirectX::FXMVECTOR color = DirectX::Colors::White, float rotation = 0, DirectX::XMFLOAT2 const& origin = Float2Zero, float scale = 1, SpriteEffects effects = SpriteEffects_None, float layerDepth = 0) const;
	void XM_CALLCONV DrawString(_In_ SpriteBatch* spriteBatch, _In_ std::wstring_view text, DirectX::XMFLOAT2 const& position, DirectX::FXMVECTOR color, float rotation, DirectX::XMFLOAT2 const& origin, DirectX::XMFLOAT2 const& scale, SpriteEffects effects = SpriteEffects_None, float layerDepth = 0) const;
	void XM_CALLCONV DrawString(_In_ SpriteBatch* spriteBatch, _In_ std::wstring_view text, DirectX::FXMVECTOR position, DirectX::FXMVECTOR color = DirectX::Colors::White, float rotation = 0, DirectX::FXMVECTOR origin = DirectX::g_XMZero, float scale = 1, SpriteEffects effects = SpriteEffects_None, float layerDepth = 0) const;
	void XM_CALLCONV DrawString(_In_ SpriteBatch* spriteBatch, _In_ std::wstring_view text, DirectX::FXMVECTOR position, DirectX::FXMVECTOR color, float rotation, DirectX::FXMVECTOR origin, DirectX::GXMVECTOR scale, SpriteEffects effects = SpriteEffects_None, float layerDepth = 0) const;

	DirectX::XMVECTOR XM_CALLCONV MeasureString(_In_ std::wstring_view text, bool ignorewhitespace = true) const;

	RECT __cdecl MeasureDrawBounds(_In_ std::wstring_view text, DirectX::XMFLOAT2 const& position, bool ignorewhitespace = true) const;
	RECT XM_CALLCONV MeasureDrawBounds(_In_ std::wstring_view text, DirectX::FXMVECTOR position, bool ignorewhitespace = true) const;
