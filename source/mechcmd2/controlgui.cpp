...
void ControlGui::render(bool bPaused)
{
	...
	if (drawGUIOn)
	{
		...
		if (bPaused)
			pauseWnd->render();
		if ((!MPlayer && (Team::home->objectives.Count() != 0) &&
				(Team::home->objectives.Status() != OS_UNDETERMINED)) ||
...
void ControlGui::renderResults()
{
	if (renderStatusInfo)
	{
		...
		{
			if (missionResultsMoveInfo[j].time <= resultsTime &&
				missionResultsMoveInfo[j + 1].time > resultsTime)
			{
...
void ControlGui::RenderObjectives()
{
	...
	if (renderObjectives || renderStatusInfo)
	{
		...
		{
			if (objectiveMoveInfo[j].time <= objectiveTime &&
				objectiveMoveInfo[j + 1].time > objectiveTime)
			{
...
bool ControlGui::getRun() { return curOrder == RUN; }
...
bool ControlGui::getJump() { return curOrder == JUMP; }
...
bool ControlGui::isDefaultSpeed() { return getWalk(); }
bool ControlGui::getWalk() { return curOrder == WALK; }
...
bool ControlGui::getGuard() { return curOrder == GUARD; }

void ControlGui::setRange(int32_t Range)
{
...
int32_t ControlGui::getCurrentRange()
{
	int32_t range = -1;
	Team* pTeam   = Team::home;
	for (size_t i = 0; i < pTeam->getRosterSize(); ++i)
...
void ControlGui::handleClick(int32_t ID)
{
	...
	case GUARDTOWER:
	{
		...
		for (size_t i = 0; i < Team::home->getRosterSize(); i++)
		{
			...
			if (pMover->isSelected() && pMover->isGuardTower() &&
...
void ControlGui::updateVehicleTab(int32_t mouseX, int32_t mouseY, bool bLOS)
{
	for (size_t i = LAST_VEHICLE - 1; i > -1; i--)
	{
		if (getButton(VEHICLE_TAB)->state & ControlButton::PRESSED)
		{
			...
			if (vehicleButtons[i].location[0].x <= mouseX &&
				mouseX <= vehicleButtons[i].location[2].x &&
				mouseY >= vehicleButtons[i].location[0].y &&
				mouseY <= vehicleButtons[i].location[1].y)
			{
				...
				int32_t lastX = mouseX - userInput->getMouseXDelta();
				int32_t lastY = mouseY - userInput->getMouseYDelta();
				if (vehicleButtons[i].location[0].x >= lastX ||
					lastX >= vehicleButtons[i].location[2].x ||
					lastY <= vehicleButtons[i].location[0].y ||
					lastY >= vehicleButtons[i].location[1].y)
					soundSystem->playDigitalSample(LOG_HIGHLIGHTBUTTONS);
			}
		}
		if (vehicleButtons[i].ID != STOP_VEHICLE)
