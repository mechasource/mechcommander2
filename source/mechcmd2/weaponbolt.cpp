...
int32_t WeaponBolt::update(void)
{
	...
	if (getFlag(OBJECT_FLAG_JUSTCREATED))
	{
		...
		if (muzzleEffect && muzzleEffect->IsExecuted())
		{
			...
			if (myOwner)
				effectRot =
					Stuff::EulerAngles(0.0f, myOwner->getRotation() * DEGREES_TO_RADS, 0.0f);
			else
...
void WeaponBolt::init(bool create, ObjectTypePtr _type)
{
	...
	if (effectId >= 0)
	{
		...
		if (strcmp(weaponEffects->GetEffectHitName(effectId), "NONE") != 0)
		{
			...
			Check_Object(gosFX::EffectLibrary::Instance);
			gosFX::Effect::Specification* gosEffectSpec =
				gosFX::EffectLibrary::Instance->Find(weaponEffects->GetEffectHitName(effectId));
			if (gosEffectSpec)
			{
				hitEffect =
					gosFX::EffectLibrary::Instance->MakeEffect(gosEffectSpec->m_effectID, flags);
			...
			useNonWeaponEffects)
		{
			...
			Check_Object(gosFX::EffectLibrary::Instance);
			gosFX::Effect::Specification* gosEffectSpec =
				gosFX::EffectLibrary::Instance->Find(weaponEffects->GetEffectMissName(effectId));
			if (gosEffectSpec)
			{
				missEffect =
					gosFX::EffectLibrary::Instance->MakeEffect(gosEffectSpec->m_effectID, flags);
			...
			(strcmp(weaponEffects->GetEffectName(WATER_MISS_FX), "NONE") != 0))
		{
			...
			Check_Object(gosFX::EffectLibrary::Instance);
			gosFX::Effect::Specification* gosEffectSpec =
				gosFX::EffectLibrary::Instance->Find(weaponEffects->GetEffectName(WATER_MISS_FX));
			if (gosEffectSpec)
			{
				waterMissEffect =
					gosFX::EffectLibrary::Instance->MakeEffect(gosEffectSpec->m_effectID, flags);
