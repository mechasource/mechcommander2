...
bool directxtk::IsValid(_In_ const WAVEFORMATEX* wfx) noexcept
{
	if (!wfx)
		return false;
	...
	case WAVE_FORMAT_ADPCM:
		...
		else
		{
			...
			bool valid = true;
			for (int32_t j = 0; j < 7 /*MSADPCM_NUM_COEFFICIENTS*/; ++j)
			{
			...
			int32_t nHeaderBytes = 7 /*MSADPCM_HEADER_LENGTH*/ * wfx->nChannels;
			int32_t nBitsPerFrame = 4 /*MSADPCM_BITS_PER_SAMPLE*/ * wfx->nChannels;
			int32_t nPcmFramesPerBlock = (wfx->nBlockAlign - nHeaderBytes) * 8 / nBitsPerFrame + 2;
...
uint32_t directxtk::GetDefaultChannelMask(int32_t channels) noexcept
{
	switch (channels)
	{
	case 1:
...
_Use_decl_annotations_ void directxtk::CreateIntegerPCM(WAVEFORMATEX* wfx, int32_t sampleRate, int32_t channels, int32_t sampleBits) noexcept
{
	int32_t blockAlign = channels * sampleBits / 8;
...
_Use_decl_annotations_ void directxtk::CreateFloatPCM(WAVEFORMATEX* wfx, int32_t sampleRate, int32_t channels) noexcept
{
	int32_t blockAlign = channels * 4;
...
_Use_decl_annotations_ void directxtk::CreateADPCM(WAVEFORMATEX* wfx, size_t wfxSize, int32_t sampleRate, int32_t channels, int32_t samplesPerBlock) noexcept(false)
{
	...
	int32_t blockAlign = (7 /*MSADPCM_HEADER_LENGTH*/) * channels
...
#ifdef DIRECTX_ENABLE_XWMA
_Use_decl_annotations_ void directxtk::CreateXWMA(WAVEFORMATEX* wfx, int32_t sampleRate, int32_t channels, int32_t blockAlign, int32_t avgBytes, bool wma3) noexcept
{
...
#ifdef DIRECTX_ENABLE_XMA2
_Use_decl_annotations_ void directxtk::CreateXMA2(WAVEFORMATEX* wfx, size_t wfxSize, int32_t sampleRate, int32_t channels, int32_t bytesPerBlock, int32_t blockCount, int32_t samplesEncoded) noexcept(false)
{
	...
	if ((bytesPerBlock < 1) || (bytesPerBlock > int32_t(XMA_READBUFFER_MAX_BYTES)))
	{
		DebugTrace("XMA2 needs a valid bytes per block\n");
		throw std::invalid_argument("XMA2WAVEFORMATEX");
	}

	int32_t blockAlign = (channels * (16 /*XMA_OUTPUT_SAMPLE_BITS*/) / 8);
	...
	xmaFmt->SamplesEncoded = static_cast<uint32_t>(samplesEncoded);
	xmaFmt->BytesPerBlock = static_cast<uint32_t>(bytesPerBlock);
...
_Use_decl_annotations_ bool directxtk::ComputePan(float pan, uint32_t channels, float* matrix) noexcept
{
...
void SoundEffectInstanceBase::Apply3D(const AudioListener& listener, const AudioEmitter& emitter, bool rhcoords)
{
	...
	if (reverb)
	{
		for (auto j = 0u; (j < mDSPSettings.SrcChannelCount) && (j < XAUDIO2_MAX_AUDIO_CHANNELS); ++j)
		{
