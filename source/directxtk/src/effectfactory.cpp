...
public:
	Impl(_In_ ID3D12Device* device, _In_ ID3D12DescriptorHeap* textureDescriptors, _In_ ID3D12DescriptorHeap* samplerDescriptors) :
		mTextureDescriptors(nullptr), mSamplerDescriptors(nullptr), mUseNormalMapEffect(true), mEnablePerPixelLighting(true), mEnableFog(false), mDevice(device), mSharing(true)
	{
...
EffectFactory::Impl::CreateEffect(
	const EffectInfo& info,
	const EffectPipelineStateDescription& opaquePipelineState,
	const EffectPipelineStateDescription& alphaPipelineState,
...
void
EffectFactory::Impl::ReleaseCache()
{
	std::lock_guard<std::mutex> lock(mutex);
	mEffectCache.clear();
...
void
EffectFactory::ReleaseCache()
{
	pImpl->ReleaseCache();
}

void
EffectFactory::SetSharing(bool enabled)
{
	pImpl->SetSharing(enabled);
}

void
EffectFactory::EnablePerPixelLighting(bool enabled)
{
	pImpl->mEnablePerPixelLighting = enabled;
}

void
EffectFactory::EnableFogging(bool enabled)
{
	pImpl->mEnableFog = enabled;
}

void
EffectFactory::EnableNormalMapEffect(bool enabled)
{
