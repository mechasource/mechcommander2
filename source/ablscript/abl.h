...
void
ABLi_init(size_t runtimeStackSize, // = 20480,
	...
	void (*ablFatalCallback)(int32_t code, const std::wstring_view& s), bool debugInfo = false, bool debug = false,
	bool profile = false);
...
int32_t ABLi_preProcess(const std::wstring_view& sourceFileName, int32_t* numErrors = nullptr,
...
void ABLi_addFunction(const std::wstring_view& name, bool isOrder, const std::wstring_view& paramList, const std::wstring_view& returnType, void (*codeCallback)(void));

void ABLi_setRandomCallbacks(void (*seedRandomCallback)(size_t seed), int32_t (*randomCallback)(int32_t range));
void ABLi_setDebugPrintCallback(void (*ABLDebugPrintCallback)(const std::wstring_view& s));
...
const std::wstring_view& ABLi_popCharPtr(void);
int32_t* ABLi_popIntegerPtr(void);
float* ABLi_popRealPtr(void);
const std::wstring_view& ABLi_popBooleanPtr(void);
...
const std::wstring_view& ABLi_peekCharPtr(void);
...
int32_t ABLi_registerInteger(const std::wstring_view& name, int32_t* address, int32_t numElements = 0);
int32_t ABLi_registerReal(const std::wstring_view& name, float* address, int32_t numElements = 0);
