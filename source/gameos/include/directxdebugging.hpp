...
PSTR __stdcall GetSendExInfo( DPID idFrom, DPID idTo, uint32_t dwFlags, uint32_t dwDataSize, uint32_t dwPriority, uint32_t dwTimeout, PVOID lpContext, puint32_t lpdwMsgID );
...
PSTR __stdcall GetDrawPrimitiveData( D3DPRIMITIVETYPE dptPrimitiveType, uint32_t dvtVertexType, PVOID lpvVertices, uint32_t dwVertexCount, uint32_t dwFlags );
PSTR __stdcall GetDrawIndexedPrimitiveData( D3DPRIMITIVETYPE d3dptPrimitiveType, uint32_t dwVertexTypeDesc, PVOID lpvVertices, uint32_t dwVertexCount, LPWORD lpwIndices, uint32_t dwIndexCount, uint32_t dwFlags );
