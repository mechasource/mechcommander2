...
// NetworkLobby.hpp should be included by the library that is performing all of
// the initial setup of games and players.  It need not be included by the
// actual application.
...
// current
//					protocol.  The string is nullptr when all sessions have been
//enumerated.  The
//					value returned is the session id.  It should be used when attempting
//to 					join a game.
//
...
//
GOSERRORCODE __stdcall gos_EnumeratePlayers(
	uint32_t session_id, LPSTRINGCALLBACK callback);
