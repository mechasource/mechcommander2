...
Spew(const std::wstring_view& group, const OBB& box)
{
	...
	SPEW((group, ""));
	SPEW((group, "  Extents = <%4f,%4f,%4f>, Radius = %4f+", box.axisExtents.x, box.axisExtents.y,
		box.axisExtents.z, box.sphereRadius));
}
#endif
...
OBB::FindSeparatingAxis(const OBB& box) const
{
	// Check_Object(this);
	Check_Object(&box);
	_ASSERT(axisExtents.x >= 0.0f);
	_ASSERT(axisExtents.y >= 0.0f);
	_ASSERT(axisExtents.z >= 0.0f);
	_ASSERT(box.axisExtents.x >= 0.0f);
	_ASSERT(box.axisExtents.y >= 0.0f);
	_ASSERT(box.axisExtents.z >= 0.0f);
	//
	//------------------------------------------
	...
	//
	float ab[3][3];
	ab[0][0] = Fabs(localToParent(0, 0) * box.localToParent(0, 0) + localToParent(0, 1) * box.localToParent(0, 1) + localToParent(0, 2) * box.localToParent(0, 2));
	ab[0][1] = Fabs(localToParent(0, 0) * box.localToParent(1, 0) + localToParent(0, 1) * box.localToParent(1, 1) + localToParent(0, 2) * box.localToParent(1, 2));
	ab[0][2] = Fabs(localToParent(0, 0) * box.localToParent(2, 0) + localToParent(0, 1) * box.localToParent(2, 1) + localToParent(0, 2) * box.localToParent(2, 2));
	float radius = axisExtents.x + box.axisExtents.x * ab[0][0] + box.axisExtents.y * ab[0][1] + box.axisExtents.z * ab[0][2];
	float projection = localToParent(0, 0) * distance.x + localToParent(0, 1) * distance.y + localToParent(0, 2) * distance.z;
	...
	//-------------
	//
	ab[1][0] = Fabs(localToParent(1, 0) * box.localToParent(0, 0) + localToParent(1, 1) * box.localToParent(0, 1) + localToParent(1, 2) * box.localToParent(0, 2));
	ab[1][1] = Fabs(localToParent(1, 0) * box.localToParent(1, 0) + localToParent(1, 1) * box.localToParent(1, 1) + localToParent(1, 2) * box.localToParent(1, 2));
	ab[1][2] = Fabs(localToParent(1, 0) * box.localToParent(2, 0) + localToParent(1, 1) * box.localToParent(2, 1) + localToParent(1, 2) * box.localToParent(2, 2));
	radius = axisExtents.y + box.axisExtents.x * ab[1][0] + box.axisExtents.y * ab[1][1] + box.axisExtents.z * ab[1][2];
	projection = localToParent(1, 0) * distance.x + localToParent(1, 1) * distance.y + localToParent(1, 2) * distance.z;
	if (projection > radius || projection < -radius)
	{
		return A1;
	}
	...
	//-------------
	//
	ab[2][0] = Fabs(localToParent(2, 0) * box.localToParent(0, 0) + localToParent(2, 1) * box.localToParent(0, 1) + localToParent(2, 2) * box.localToParent(0, 2));
	ab[2][1] = Fabs(localToParent(2, 0) * box.localToParent(1, 0) + localToParent(2, 1) * box.localToParent(1, 1) + localToParent(2, 2) * box.localToParent(1, 2));
	ab[2][2] = Fabs(localToParent(2, 0) * box.localToParent(2, 0) + localToParent(2, 1) * box.localToParent(2, 1) + localToParent(2, 2) * box.localToParent(2, 2));
	radius = axisExtents.z + box.axisExtents.x * ab[2][0] + box.axisExtents.y * ab[2][1] + box.axisExtents.z * ab[2][2];
	projection = localToParent(2, 0) * distance.x + localToParent(2, 1) * distance.y + localToParent(2, 2) * distance.z;
	if (projection > radius || projection < -radius)
	{
		return A2;
	}
	...
	//-------------
	//
	radius = box.axisExtents.x + axisExtents.x * ab[0][0] + axisExtents.y * ab[1][0] + axisExtents.z * ab[2][0];
	projection = box.localToParent(0, 0) * distance.x + box.localToParent(0, 1) * distance.y + box.localToParent(0, 2) * distance.z;
	if (projection > radius || projection < -radius)
	{
		return B0;
	}
	...
	//-------------
	//
	radius = box.axisExtents.y + axisExtents.x * ab[0][1] + axisExtents.y * ab[1][1] + axisExtents.z * ab[2][1];
	projection = box.localToParent(1, 0) * distance.x + box.localToParent(1, 1) * distance.y + box.localToParent(1, 2) * distance.z;
	if (projection > radius || projection < -radius)
	{
		return B1;
	}
	...
	//-------------
	//
	radius = box.axisExtents.z + axisExtents.x * ab[0][2] + axisExtents.y * ab[1][2] + axisExtents.z * ab[2][2];
	projection = box.localToParent(2, 0) * distance.x + box.localToParent(2, 1) * distance.y + box.localToParent(2, 2) * distance.z;
	if (projection > radius || projection < -radius)
	{
		return B2;
	}
	...
	radius = axisExtents.y * ba[0][2] + axisExtents.z * ba[0][1] + box.axisExtents.y * minor[2][0] + box.axisExtents.z * minor[1][0];
	...
	radius = axisExtents.y * ba[1][2] + axisExtents.z * ba[1][1] + box.axisExtents.x * minor[2][0] + box.axisExtents.z * minor[0][0];
	...
	radius = axisExtents.y * ba[2][2] + axisExtents.z * ba[2][1] + box.axisExtents.x * minor[1][0] + box.axisExtents.y * minor[0][0];
	...
	radius = axisExtents.x * ba[0][2] + axisExtents.z * ba[0][0] + box.axisExtents.y * minor[2][1] + box.axisExtents.z * minor[1][1];
	...
	radius = axisExtents.x * ba[1][2] + axisExtents.z * ba[1][0] + box.axisExtents.x * minor[2][1] + box.axisExtents.z * minor[0][1];
	...
	radius = axisExtents.x * ba[2][2] + axisExtents.z * ba[2][0] + box.axisExtents.x * minor[1][1] + box.axisExtents.y * minor[0][1];
	...
	radius = axisExtents.x * ba[0][1] + axisExtents.y * ba[0][0] + box.axisExtents.y * minor[2][2] + box.axisExtents.z * minor[1][2];
	...
	radius = axisExtents.x * ba[1][1] + axisExtents.y * ba[1][0] + box.axisExtents.x * minor[2][2] + box.axisExtents.z * minor[0][2];
	...
	//-----------------------
	//
	radius = axisExtents.x * ba[2][1] + axisExtents.y * ba[2][0] + box.axisExtents.x * minor[1][2] + box.axisExtents.y * minor[0][2];
