...
bool
DirectX::IsValid(_In_ const WAVEFORMATEX* wfx)
{
	if (!wfx)
		return false;
...
uint32_t
DirectX::GetDefaultChannelMask(int channels)
{
	switch (channels)
	{
	case 1:
...
DirectX::CreateIntegerPCM(WAVEFORMATEX* wfx, int sampleRate, int channels, int sampleBits)
{
...
_Use_decl_annotations_ void
DirectX::CreateFloatPCM(WAVEFORMATEX* wfx, int sampleRate, int channels)
{
...
_Use_decl_annotations_ void
DirectX::CreateADPCM(WAVEFORMATEX* wfx, size_t wfxSize, int sampleRate, int channels, int samplesPerBlock)
{
...
#if defined(_XBOX_ONE) || (_WIN32_WINNT < _WIN32_WINNT_WIN8) || (_WIN32_WINNT >= _WIN32_WINNT_WIN10)
_Use_decl_annotations_ void
DirectX::CreateXWMA(WAVEFORMATEX* wfx, int sampleRate, int channels, int blockAlign, int avgBytes, bool wma3)
{
...
void
SoundEffectInstanceBase::Apply3D(const AudioListener& listener, const AudioEmitter& emitter, bool rhcoords)
{
	if (!voice)
		return;
