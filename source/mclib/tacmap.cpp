	...
	int32_t ySize, gos_VERTEX& tac)
{
	...
	tac.x = (float)tacX * (float)xSize / ((float)land->realVerticesMapSide * terrain_const::MAPCELL_DIM);
	tac.y = (float)tacY * (float)ySize / ((float)land->realVerticesMapSide * terrain_const::MAPCELL_DIM);
	tac.x += xOffset;
	tac.y += yOffset;
}
	...
	const Stuff::Vector2DOf<int32_t>& screen, int32_t xSize, int32_t ySize, Stuff::Vector3D& world)
{
	...
	int32_t cellX =
		screen.x / (float)xSize * ((float)land->realVerticesMapSide * terrain_const::MAPCELL_DIM);
	int32_t cellY =
		screen.y / (float)ySize * ((float)land->realVerticesMapSide * terrain_const::MAPCELL_DIM);
	if ((cellX < 0) || (cellX >= land->realVerticesMapSide * terrain_const::MAPCELL_DIM) || (cellY < 0) || (cellY >= land->realVerticesMapSide * terrain_const::MAPCELL_DIM))
	{
