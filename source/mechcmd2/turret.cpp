...
GameObjectPtr
TurretType::createInstance(void)
{
...
bool
TurretType::handleCollision(GameObjectPtr collidee, GameObjectPtr collider)
{
	...
	if (target)
	{
		...
		targetRange =
			targetRangeV3.x * targetRangeV3.x + targetRangeV3.y * targetRangeV3.y; //().magnitude();
	}
	//-------------------------------------
...
bool
TurretType::handleDestruction(GameObjectPtr collidee, GameObjectPtr collider)
{
	return (false);
}
...
int32_t
Turret::setTeamId(int32_t _teamId, bool setup)
{
	if (MPlayer)
	{
		teamId = _teamId;
...
bool
Turret::isFriendly(TeamPtr team)
{
	if (teamId > -1)
...
//---------------------------------------------------------------------------
void
Turret::setDamage(float newDamage)
{
	damage = newDamage;
...
Turret::update(void)
{
	...
	if (getFlag(OBJECT_FLAG_DESTROYED))
	{
		...
		appearance->setObjectParameters(
			position, rotation, drawFlags, teamId, Team::getRelation(teamId, Team::home->getId()));
	...
	if (targetWID && (!MPlayer || MPlayer->isServer()))
	{
		...
		if (attackTarget)
		{
			...
			if (attackTarget->isDisabled() || attackTarget->isDestroyed() || GameObject::isFriendly(attackTarget) || (targetRange > (ourExtentRadius * ourExtentRadius)))
				targetWID = 0;
		}
		else
			targetWID = 0;
	...
	// linkage code!
	if (active && parent && (ObjectManager->getByWatchID(parent)->isDestroyed() || ObjectManager->getByWatchID(parent)->isDisabled() || !ObjectManager->getByWatchID(parent)->getAwake()))
	{
	...
	if (active && parent && !ObjectManager->getByWatchID(parent)->isDestroyed() && !ObjectManager->getByWatchID(parent)->isDisabled() && (ObjectManager->getByWatchID(parent)->getTeamId() != getTeamId()))
	{
...
float
Turret::relFacingTo(Stuff::Vector3D goal, int32_t bodyLocation)
{
...
bool
Turret::isWeaponReady(int32_t weaponId)
{
...
bool
Turret::isWeaponMissile(int32_t weaponId)
{
	if (((TurretTypePtr)getObjectType())->weaponMasterId[weaponId] == -1)
		return false;
	return (MasterComponent::masterList[((TurretTypePtr)getObjectType())->weaponMasterId[weaponId]]
				.getForm()
...
bool
Turret::isWeaponStreak(int32_t weaponId)
{
	if (((TurretTypePtr)getObjectType())->weaponMasterId[weaponId] == -1)
		return false;
	return (MasterComponent::masterList[((TurretTypePtr)getObjectType())->weaponMasterId[weaponId]]
...
float
Turret::calcAttackChance(GameObjectPtr target, int32_t* range, int32_t weaponId)
{
	if (((TurretTypePtr)getObjectType())->weaponMasterId[weaponId] == -1)
		return 0.0f;
	//-------------------------------------------------------------
	...
	float weaponMinRange =
		WeaponRanges[MasterComponent::masterList[weaponMasterId].getWeaponRange()][0];
	float weaponMaxRange =
		WeaponRanges[MasterComponent::masterList[weaponMasterId].getWeaponRange()][1];
...
void
Turret::recordWeaponFireTime(int32_t weaponId)
{
...
void
Turret::startWeaponRecycle(int32_t weaponId)
{
...
int32_t
Turret::clearWeaponFireChunks(int32_t which)
{
...
int32_t
Turret::addWeaponFireChunk(int32_t which, WeaponFireChunkPtr chunk)
{
...
int32_t
Turret::addWeaponFireChunks(int32_t which, uint32_t* packedChunkBuffer, int32_t numChunks)
{
...
int32_t
Turret::grabWeaponFireChunks(int32_t which, uint32_t* packedChunkBuffer)
{
...
int32_t
Turret::updateWeaponFireChunks(int32_t which)
{
	for (size_t i = 0; i < numWeaponFireChunks[which]; i++)
	{
		WeaponFireChunk chunk;
...
Stuff::Vector3D
Turret::getPositionFromHS(int32_t nodeId)
{
	//-----------------------------------------------------
...
void
Turret::printHandleWeaponHitDebugInfo(WeaponShotInfo* shotInfo)
{
	if (!CombatLog)
		return;
	static const std::wstring_view& locationStrings[] = {"head", "center torso", "left torso", "right torso",
	...
	// sprintf(statusStr, " NO PAIN:");
		...
		sprintf(statusStr, ":");
	sprintf(s, "[%.2f] turret.handleWeaponHit%s (%05d)%s ", scenarioTime, statusStr, getPartId(),
		getName());
	CombatLog->write(s);
...
void
Turret::fireWeapon(GameObjectPtr target, int32_t weaponId)
{
	if (!target)
		return;
	int32_t weaponMasterId = ((TurretTypePtr)getObjectType())->weaponMasterId[weaponId];
	...
	if (hitRoll < attackChance)
	{
		...
		if (MasterComponent::masterList[weaponMasterId].getForm() == COMPONENT_FORM_WEAPON_MISSILE)
		{
			...
			int32_t missileAmount =
				MasterComponent::masterList[weaponMasterId].getWeaponAmmoAmount();
			int32_t numMissiles;
			if (isStreakMissile)
			...
			// numClusters = 1;
			uint8_t effectType =
				MasterComponent::masterList[weaponMasterId].getWeaponSpecialEffect();
			//----------------------------------------------------
				...
				if (target)
				{
					if (aimLocation == -1)
						hitLocation = target->calcHitLocation(
							this, weaponMasterId, ATTACKSOURCE_WEAPONFIRE, ATTACK_TO_DESTROY);
					...
					if (target)
					{
						if (target->isMover())
							chunk.buildMoverTarget(
								target, weaponId, true, entryAngle, numMissiles, hitLocation);
						else
							chunk.buildTerrainTarget(target, weaponId, true, numMissiles);
						...
						if (!chunk.equalTo(&chunk2))
							Fatal(0,
								" Turret.fireWeapon: Bad WeaponFireChunk "
		...
		else
		{
			...
			shotInfo.init(this->getWatchID(), weaponMasterId,
				MasterComponent::masterList[weaponMasterId].getWeaponDamage(), hitLocation,
				entryAngle);
			//-------------------------------------------------------------------------
			// If I'm in a multiplayer game and I'm the server, record this
			// turret fire so it may be broadcast to all clients...
			if (MPlayer && MPlayer->isServer())
			{
				...
				{
					if (target->isMover())
						chunk.buildMoverTarget(target, weaponId, true, entryAngle, 0, hitLocation);
					else
			...
			if (MasterComponent::masterList[weaponMasterId].getForm() == COMPONENT_FORM_WEAPON_MISSILE)
			{
				...
				if (numMissiles > 0)
				{
						...
						if (!canSeeHit)
						{
							...
							if (!canSeeHit)
							{
								...
								if (!canSeeHit)
								{
									...
									curShotInfo.init(
										this->getWatchID(), weaponMasterId, 0.0f, -1, entryAngle);
								}
							}
						}
					}
					...
					printFireWeaponDebugInfo(
						target, &positionOffset, attackChance, hitRoll, &curShotInfo);
				}
			}
			else
			{
				...
				WeaponShotInfo shotInfo;
				shotInfo.init(this->getWatchID(), weaponMasterId,
					MasterComponent::masterList[weaponMasterId].getWeaponDamage(), -1, entryAngle);
				uint8_t effectType =
					MasterComponent::masterList[weaponMasterId].getWeaponSpecialEffect();
				WeaponBoltPtr weaponFX = ObjectManager->createWeaponBolt(effectType);
				if (!weaponFX)
...
void
Turret::lightOnFire(float timeToBurn)
{
}

//---------------------------------------------------------------------------
void
Turret::renderShadows(void)
{
	...
	if (appearance->canBeSeen())
	{
		...
		if (((TurretTypePtr)getObjectType())->turretTypeName < IDS_MC2_STRING_START)
		{
			appearance->setObjectNameId(
				((TurretTypePtr)getObjectType())->turretTypeName + IDS_MC2_STRING_START);
		}
		else
		{
...
void
Turret::init(bool create, ObjectTypePtr _type)
{
	//-------------------------------------------
...
int32_t
Turret::handleWeaponHit(WeaponShotInfoPtr shotInfo, bool addMultiplayChunk)
{
	if (!shotInfo)
		return (NO_ERROR);
...
//---------------------------------------------------------------------------
void
Turret::setParentId(uint32_t pId)
{
	parentId = pId;
}
...
Turret::updateDebugWindow(GameDebugWindow* debugWindow)
{
	...
	sprintf(
		s, "team: %d, partID: %d, objT: %d", getTeamId(), getPartId(), getObjectType()->whatAmI());
	debugWindow->print(s);
	GameObjectPtr target = ObjectManager->getByWatchID(targetWID);
...
//***************************************************************************
void
Turret::Save(PacketFilePtr file, int32_t packetNum)
{
	TurretData data;
	CopyTo(&data);
