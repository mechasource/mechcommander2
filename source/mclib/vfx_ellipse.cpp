...
static int32_t paneY0, paneY1, paneX0, paneX1, DestWidth, DestBuffer, x_top,
	y_top, Bsquared, TwoBsquared, Asquared, TwoAsquared, var_dx, var_dy,
	x_vector, line_left, line_right;
static int32_t DrawRoutine;

void AG_ellipse_draw(PANE* pane, int32_t xc, int32_t yc, int32_t width,
	int32_t height, int32_t color)
{
	if (width == 0 || height == 0)
	{
		VFX_line_draw(
			pane, xc - width, yc - height, xc + width, yc + height, 0, color);
		return;
	}
...
void AG_ellipse_fill(PANE* pane, int32_t xc, int32_t yc, int32_t width,
	int32_t height, int32_t color)
{
	if (width == 0 || height == 0)
	{
...
void AG_ellipse_fillOr(PANE* pane, int32_t xc, int32_t yc, int32_t width,
	int32_t height, int32_t color)
{
	if (width == 0 || height == 0)
	{
		...
		VFX_line_draw(pane, xc - width, yc - height, xc + width, yc + height,
			LD_EXECUTE, (int32_t)(&orLineCallback));
		return;
	}
	DestWidth	  = pane->window->x_max + 1;
	int32_t paneX0 = (pane->x0 < 0) ? 0 : pane->x0;
...
void AG_ellipse_fillXor(PANE* pane, int32_t xc, int32_t yc, int32_t width,
	int32_t height, int32_t color)
{
	...
	if (width == 0 || height == 0)
	{
		...
		gColor   = (uint8_t)color;
		VFX_line_draw(pane, xc - width, yc - height, xc + width, yc + height,
			LD_EXECUTE, (int32_t)(&andLineCallback));
		return;
	}
	DestWidth	  = pane->window->x_max + 1;
	int32_t paneX0 = (pane->x0 < 0) ? 0 : pane->x0;
