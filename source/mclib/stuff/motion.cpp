...
//###########################################################################
//
Motion3D& Motion3D::operator=(const Motion3D& motion)
{
	// Check_Pointer(this);
	Check_Object(&motion);
	angularMotion = motion.angularMotion;
...
//
#if !defined(Spew)
void Spew(PCSTR group, const Motion3D& motion)
{
	Check_Object(&motion);
	SPEW((group, "{+"));
...
//###########################################################################
//
void Motion3D::TestInstance(void) const { Check_Object(&angularMotion); }
