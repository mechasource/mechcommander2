...
ABLi_init(size_t runtimeStackSize, // = 20480,
	...
	void (*ablFatalCallback)(int32_t code, PSTR s), bool debugInfo = false, bool debug = false,
	bool profile = false);
...
int32_t
ABLi_execute(SymTableNodePtr moduleIdPtr, SymTableNodePtr functionIdPtr = nullptr,
	ABLParamPtr paramList = nullptr, StackItemPtr returnVal = nullptr);

int32_t
ABLi_deleteModule(SymTableNodePtr moduleIdPtr);
...
ABLi_setRandomCallbacks(
	void (*seedRandomCallback)(size_t seed), int32_t (*randomCallback)(int32_t range));
void
ABLi_setDebugPrintCallback(void (*ABLDebugPrintCallback)(PSTR s));
...
int32_t
ABLi_registerInteger(PSTR name, int32_t* address, int32_t numElements = 0);
int32_t
ABLi_registerReal(PSTR name, float* address, int32_t numElements = 0);
...
void
ABLi_transState(int32_t newState);
