...
PSTR __stdcall GetSendExInfo(DPID idFrom, DPID idTo, uint32_t dwFlags,
	uint32_t dwDataSize, uint32_t dwPriority, uint32_t dwTimeout,
	PVOID lpContext, puint32_t lpdwMsgID);
PSTR __stdcall GetSendInfo(
	DPID idFrom, DPID idTo, uint32_t dwFlags, uint32_t dwDataSize);
PSTR __stdcall GetReceivedData(
	LPDPID lpidFrom, PVOID lpData, puint32_t lpdwDataSize);
PSTR __stdcall GetReceiveFlags(
	uint32_t dwFlags, LPDPID lpidFrom, LPDPID lpidTo);
...
PSTR __stdcall GetDrawPrimitiveData(D3DPRIMITIVETYPE dptPrimitiveType,
	uint32_t dvtVertexType, PVOID lpvVertices, uint32_t dwVertexCount,
	uint32_t dwFlags);
PSTR __stdcall GetDrawIndexedPrimitiveData(D3DPRIMITIVETYPE d3dptPrimitiveType,
	uint32_t dwVertexTypeDesc, PVOID lpvVertices, uint32_t dwVertexCount,
	LPWORD lpwIndices, uint32_t dwIndexCount, uint32_t dwFlags);
PSTR __stdcall GetClearArea(
	uint32_t dwCount, LPD3DRECT lpRects, uint32_t dwFlags);
...
PSTR __stdcall GetBltInformation(LPDIRECTDRAWSURFACE7 lpdd7, LPRECT lpDestRect,
	LPDIRECTDRAWSURFACE7 lpDDSrcSurface, LPRECT lpSrcRect, uint32_t dwFlags,
	LPDDBLTFX lpDDBltFx);
PSTR __stdcall GetBltFastInformation(LPDIRECTDRAWSURFACE7 lpdd7, uint32_t dwX,
