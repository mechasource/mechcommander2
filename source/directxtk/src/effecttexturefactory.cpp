...
public:
	...
	Impl(
		...
		mPath{},
		mTextureDescriptorHeap(descriptorHeap), mDevice(device), mResourceUploadBatch(resourceUploadBatch), mSharing(true), mForceSRGB(false), mAutoGenMips(false)
	{
		*mPath = 0;
	}
	...
	Impl(
		...
		mPath{},
		mTextureDescriptorHeap(device, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, descriptorHeapFlags, numDescriptors), mDevice(device), mResourceUploadBatch(resourceUploadBatch), mSharing(true), mForceSRGB(false), mAutoGenMips(false)
	{
...
void
EffectTextureFactory::Impl::ReleaseCache()
{
	std::lock_guard<std::mutex> lock(mutex);
	mTextureCache.clear();
}
...
void
EffectTextureFactory::ReleaseCache()
{
	pImpl->ReleaseCache();
}

void
EffectTextureFactory::SetSharing(bool enabled)
{
	pImpl->SetSharing(enabled);
}

void
EffectTextureFactory::EnableForceSRGB(bool forceSRGB)
{
	pImpl->EnableForceSRGB(forceSRGB);
}

void
EffectTextureFactory::EnableAutoGenMips(bool generateMips)
{
...
EffectTextureFactory::GetCpuDescriptorHandle(size_t index) const
{
	return pImpl->mTextureDescriptorHeap.GetCpuHandle(index);
}

D3D12_GPU_DESCRIPTOR_HANDLE
EffectTextureFactory::GetGpuDescriptorHandle(size_t index) const
{
...
EffectTextureFactory::GetResource(size_t slot, ID3D12Resource** resource, bool* isCubeMap)
{
