...
getNumber(void)
{
	...
	if (curLiteral.type == LIT_INTEGER)
	{
		...
		//		if (((int32_t)numberValue < -MAX_INTEGER) || ((int32_t)numberValue
		//> MAX_INTEGER))
		//		{
...
getSpecial(void)
{
	*tokenp = curChar;
	...
	case '#':
		...
		getChar();
		break;
		// NOTE: For now, let's use the BEGIN and END keywords instead of
		// braces. If popular demand is to get rid of the begin/end keywords and
		// switch to braces, we can do so...
		//		case '{':
		//			curToken = TKN_BEGIN;
		//			getChar();
		//			break;
